...
//사무업무 : 박소향 (직무교육, 전기요금컨설팅, 전기담당) 전기담당 ID 변경 kcmc1001 -> electric 160919 / 4대보험 161201
else if($member['mb_id'] == "kcmc1006") $sql_search .= " or user_id='$mb_id' or alert_code='80003' or send_to like '%electric%' or send_to like '%si4n%' or send_to like '%kcmc1001%' ) ";
//지원금관리 : 전정애 (계약서, 지원금)
else if($member['mb_id'] == "kcmc1008") $sql_search .= " or send_to like '%manager%' or alert_code='10003' ) ";
//사무위탁 : 임현미 / 15000 위탁서접수 / 전기담당 / 4대보험 161201
else if($member['mb_id'] == "kcmc1007") $sql_search .= " or alert_code='15000' or send_to like '%electric%' or send_to like '%si4n%' ) ";
...
//한국기업경영원 : 김근호 160602 -> 이경훈 / kcmc1001 -> electric 제외 161004 / electric 포함 / 전기공사 전달사항 표시 161207
else if($member['mb_id'] == "kcmc1003") $sql_search .= " or user_id='$mb_id'  or send_to like '%electric%' or memo_type=99 ) ";
//한국기업경영원 : 김미정 160822
else if($member['mb_id'] == "kcmc1012") $sql_search .= " or user_id='$mb_id'  or send_to like '%kcmc1001%' ) ";
//이지노무/가족보험 : 김국진 : 대상자(send_to) 사업주훈련(김소향 kcmc1006) 알림 표시 150915 / kcmc1006(1년 전기요금절감 요청 알림) 제거 160411 / 담당자 / 4대보험 161201
else if($member['mb_id'] == "kcmc1009") $sql_search .= " or user_id='$mb_id' or send_to like '%electric%' or send_to like '%si4n%' or alert_code='50001' or alert_code='10004' or alert_code='10005' or manage_code=26 ) ";
//ERP문의 : 이영래
...
//접수, 업무 알림 구분
$stx_receive_arry = array("","의뢰서","계약서","건설업체","가족보험","전기요금","위탁서","대리인","전자민원","시간선택제","고용창출","신규고용","이지노무","직무교육","4대보험절감");
$stx_work_arry = array("","신청서류","보완서류","우편물발송","검토현황","지원금접수","전달첨부서류","삭제요청","업무일지","근태신청","시말서","휴가신청");
//접수 체크박스 갯수 160428 / 4대보험절감 추가 13->14 161206
$stx_receive_cnt = 14;
//업무 체크박스 갯수 / 업무일지, 근태신청, 시말서, 휴가신청 추가 160429
$stx_work_cnt = 11;

//접수, 업무 알림구분 체크 시 where 쿼리 추가
if($stx_receive_1 || $stx_receive_2 || $stx_receive_3 || $stx_receive_4 || $stx_receive_5 || $stx_receive_6 || $stx_receive_7 || $stx_receive_8 || $stx_receive_9 || $stx_receive_10 || $stx_receive_11 || $stx_receive_12 || $stx_receive_13 || $stx_receive_14 || $stx_work_1 || $stx_work_2 || $stx_work_3 || $stx_work_4 || $stx_work_5 || $stx_work_6 || $stx_work_7 || $stx_work_8 || $stx_work_9 || $stx_work_10 || $stx_work_11) {
	$search_or = "";
	$sql_search .= " and ( ";
	if($stx_receive_1) {
	...
	if($stx_receive_14) {
		$sql_search_add[14] = $search_or." alert_code =10008 ";
		$search_or = "or";
	}
	...
	if($stx_work_8) {
		$sql_search_add2[8] = $search_or." alert_code =90010 ";
		$search_or = "or";
	}
	if($stx_work_9) {
		$sql_search_add2[9] = $search_or." alert_code =90012 ";
		$search_or = "or";
	}
	if($stx_work_10) {
		$sql_search_add2[10] = $search_or." alert_code =90013 ";
		$search_or = "or";
	}
	if($stx_work_11) {
		$sql_search_add2[11] = $search_or." alert_code =90014 ";
		$search_or = "or";
	}
...
//대상자 : 총괄관리 kcmc1001 / 전기담당 electric 160920
...
$qstr .= "&stx_memo=".$stx_memo."&stx_send_to1=".$stx_send_to1."&stx_send_to2=".$stx_send_to2."&stx_send_to3=".$stx_send_to3."&stx_send_to4=".$stx_send_to4."&stx_send_to5=".$stx_send_to5."&stx_send_to6=".$stx_send_to6."&stx_send_to7=".$stx_send_to7."&stx_send_to8=".$stx_send_to8;
//알림구분 접수, 업무
//$qstr .= "&amp;stx_receive_use=".$stx_receive_use;
for($i=1;$i<=$stx_receive_cnt;$i++) {
...
//전체 체크 기능 하단에 제공 : 부장님 의견 160520
function checkAll2() {
	var f = document.dataForm;
	for( var i = 0; i<f.idx.length; i++ ) {
		f.idx[i].checked = f.checkall2.checked;
	}
}
function del_ok() {
									...
									<td valign="top" style="padding:10px 0 0 0">
										...
										<form name="searchForm" method="get">
											...
											<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable1" style="">
												<tr>
													<td nowrap class="tdrowk" width="84"><img src="images/icon_02.gif" width="2" height="2" style="margin:0 5px 3px 0">사업장명</td>
													...
													<td nowrap class="tdrow" width="140">
														<select name="stx_alert_code" class="selectfm">
															...
															<option value="10007" <? if($stx_alert_code == '10007') echo "selected"; ?>>직무발명보상</option>
															<option value="10008" <? if($stx_alert_code == '10008') echo "selected"; ?>>4대보험절감</option>
...
?>
												...
												<tr>
													...
													<td nowrap class="tdrowk"><img src="images/icon_02.gif" width="2" height="2" style="margin:0 5px 3px 0">대상자</td>
													<td nowrap class="tdrow" colspan="<?=$search_colspan2?>">
														<input type="checkbox" name="stx_send_to2" value="kcmc1001" <? if($stx_send_to2 == "kcmc1001") echo "checked"; ?> style="vertical-align:middle" />총괄관리
														<input type="checkbox" name="stx_send_to7" value="electric" <? if($stx_send_to7 == "electric") echo "checked"; ?> style="vertical-align:middle" />전기담당
														<input type="checkbox" name="stx_send_to8" value="kcmc1006" <? if($stx_send_to8 == "kcmc1006") echo "checked"; ?> style="vertical-align:middle" />사업주훈련
														<input type="checkbox" name="stx_send_to1" value="kcmc1007" <? if($stx_send_to1 == "kcmc1007") echo "checked"; ?> style="vertical-align:middle" />사무위탁
														<input type="checkbox" name="stx_send_to3" value="kcmc1008" <? if($stx_send_to3 == "kcmc1008") echo "checked"; ?> style="vertical-align:middle" />계약서/대리인선임
														<input type="checkbox" name="stx_send_to4" value="manager"  <? if($stx_send_to4 == "manager")  echo "checked"; ?> style="vertical-align:middle" />지원금/부담금
														<input type="checkbox" name="stx_send_to5" value="kcmc1009" <? if($stx_send_to5 == "kcmc1009") echo "checked"; ?> style="vertical-align:middle" />이지노무/가족보험
														<input type="checkbox" name="stx_send_to6" value="branch"   <? if($stx_send_to6 == "branch")   echo "checked"; ?> style="vertical-align:middle" />담당지사
													</td>
												</tr>
											</table>
...
?>
										...
										<form name="dataForm" method="post">
											...
											<input type="hidden" name="stx_read_my"  value="<?=$stx_read_my?>" />
											<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:fixed">
												<tr>
													...
													<td class="tdhead_center" width="74" rowspan="">등록일시</td>
...
*/
	...
	for($sta=0; $sta<=count($send_to_array);$sta++) {
		if($send_to_array[0]) {
			...
			else if($send_to_array[$sta] == "kcmc1008") $send_to .= "지원금관리";
			...
			else if($send_to_array[$sta] == "electric") $send_to .= "전기담당";
			else if($send_to_array[$sta] == "si4n") $send_to .= "4대보험";
			else if($send_to_array[$sta] == "kcmc2007") $send_to .= "마케팅";
			else if($send_to_array[$sta] == "kcmc2006") $send_to .= "전산실";
			else if($send_to_array[$sta] == "kcmc1004") $send_to .= "관리부장";
			else if($send_to_array[$sta] == "kcmc0331") $send_to .= "영업관리1";
			else if($send_to_array[$sta] == "kcmc2001") $send_to .= "영업관리2";
	...
	//등록시간
	$date2 = substr($row['wr_datetime'],11,8); //날짜표시형식변경
	$latest_time = $date2;
...
?>
												<tr>
													<td class="tdhead_center"><input type="checkbox" name="checkall2" value="1" class="textfm" onclick="checkAll2()" ></td>
