...
//대표자HP
$to_phone = $row_com['boss_hp'];

//전기요금 컨설팅 고객관리 DB
$sql_manage = " select * from electric_manage where idx='$idx' ";
$result_manage = sql_query($sql_manage);
$row_manage = mysql_fetch_array($result_manage);

//메시지
$sms_message = "에너지절약 전문컨설팅 [한국기업경영원]입니다. ".$row_manage['month']."월전기절감액은 ".$row_manage['electric_before_contrast']."원입니다.";
?>
	...
	if (frm.to_hp.value == "") {
	 ...
	 return;
		...
		if(confirm("정말 문자 발송 하시겠습니까?")) {
			frm.action="popup_sms_update_electric.php";
			frm.submit();
		} else {
			return;
		}
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable1" style="">
		<tr>
			...
			<td nowrap class="tdrow">
				<textarea id="chk1" name="chk1" onkeyup="displayBytes(80,'chk1');" style="width:228px;height:150px;font-size:18pt;"><?=$sms_message?></textarea>
				<br />
