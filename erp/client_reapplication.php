...
if(!$search_ok) {
	...
	$sql_search .= " and (c.reapplication_date >= '$this_month_start') ";
...
//재접수일자 존재 거래처만 검색
$sql_search .= " and ( c.reapplication_date != '' ) ";
//검색 : 사업장명칭
if($stx_comp_name) {
	$sql_search .= " and ( ";
...
//접수여부
if($stx_application_accept_chk) {
	$sql_search .= " and ( ";
	if($stx_application_accept_chk == 1) {
		$sql_search .= " (c.application_accept != '') ";
	} else if($stx_application_accept_chk == 2) {
		$sql_search .= " (c.application_accept = '') ";
	}
	$sql_search .= " ) ";
}
			...
			<div style="margin:10px 20px 20px 20px;min-height:480px;">
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						...
						<td width=""><img src="images/top02_03.gif" border="0"></td>
						<td>
<?
$title_main_no = "02";
...
<? } ?>
									...
									<tr>
										...
										<td nowrap class="tdrowk"><img src="images/icon_02.gif" width="2" height="2" style="margin:0 5px 3px 0">접수여부</td>
										<td nowrap class="tdrow" colspan="3">
											<select name="stx_application_accept_chk" class="selectfm" onchange="">
												<option value=""  <? if($stx_application_accept_chk == "")  echo "selected"; ?>>전체</option>
												<option value="1" <? if($stx_application_accept_chk == "1") echo "selected"; ?>>접수</option>
												<option value="2" <? if($stx_application_accept_chk == "2") echo "selected"; ?>>미접수</option>
											</select>
										</td>
									</tr>
									<tr>
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
  ...
  $list = $i%2;
	...
	//재접수일자 색상
	if($search_day1) {
		if($reapplication_date >= $search_sday && $reapplication_date <= $search_eday) $reapplication_date_color = "color:red;font-weight:bold";
	...
	if($member['mb_level'] > 6 || $row['damdang_code'] == $member['mb_profile']) {
		$com_view = "javascript:alert('지원금 정보 수정을 위해 지원금현황 페이지로 이동합니다.');self.location.href='client_application_view.php?id=$id&w=u&$qstr&page=$page';";
	} else {
...
?>
									<tr class="list_row_now_wh" onMouseOver="this.className='list_row_over';" onMouseOut="this.className='list_row_now_wh';">
										...
										<td class="ltrow1_center_h22" style="<?=$tax_invoice_color?>"><?=$tax_invoice?></td>
									</tr>
<?
}
if ($i == 0)
...
?>
									<tr>
										...
										<td class="tdhead_center"></td>
										<td class="tdhead_center"></td>
										<td class="tdhead_center" colspan="5">합계(신청금액, 매출금액)</td>
										<td class="tdhead_center"></td>
