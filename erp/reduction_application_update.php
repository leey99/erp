...
//첨부파일 경로
$upload_dir = 'files/reduction_app_file/';
...
";
// 데이터2 유무
$sql2 = " select * from com_reduction_app where com_code='$id' ";
$result2 = sql_query($sql2);
$row2 = mysql_fetch_array($result2);
//데이터2 존재 시 update, 미존재 시 insert
if($row2['com_code']) {
	$sql_opt2 = " update com_reduction_app set 
				$sql_common_opt2 
				where com_code = '$id' ";
} else {
	$sql_opt2 = " insert com_reduction_app set 
				$sql_common_opt2 
				, com_code = '$id' ";
}
sql_query($sql_opt2);
//완료 후 페이지 이동
alert("정상적으로 감정방지기간조회 정보가 수정 되었습니다.","reduction_application_view.php?id=".$id."&w=".$w."&page=".$page."&".$qstr);
?>