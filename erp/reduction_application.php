...
if(!$stx_comp_name && !$stx_biz_no) {
	$sql_common = " from com_list_gy a, com_list_gy_opt b, com_reduction_app c, com_list_gy_opt2 d ";
} else {
...
function goCheck_ok(obj) {
	...
	check_ok_iframe.location.href = "reduction_application_check_ok_update.php?id="+id+"&check_ok="+check_ok;
	return;
}
<?
//검색기간 함수
...
$sql_reduction = " select c.reduction_process from com_list_gy a, com_reduction_app c where a.com_code=c.com_code and c.delete_yn != '1' $sql_search_add $sql_search_add2 ";
//echo $sql_reduction;
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
  ...
  $list = $i%2;
	...
	if($member['mb_level'] > 6 || $row['damdang_code'] == $member['mb_profile'] || $row['damdang_code2'] == $member['mb_profile']) {
		$com_view = "reduction_application_view.php?id=$id&amp;w=u&amp;page=$page&amp;$qstr";
	} else {
	...
	if($stx_comp_name || $stx_biz_no) {
		$sql_reduction = " select * from com_reduction_app where com_code='$id' ";
		$result_reduction = sql_query($sql_reduction);
...
<iframe name="check_ok_iframe" src="reduction_application_check_ok_update.php" style="width:0;height:0" frameborder="0"></iframe>
