...
<? } ?>
								...
								<tr>
									<td nowrap class="tdrowk"><img src="images/icon_02.gif" width="2" height="2" style="margin:0 5px 3px 0;">분납횟수</td>
									<td nowrap class="tdrow" colspan="">
...
?>
									...
									<td class="tdhead_center" width="62">급여대장</td>
									<td class="tdhead_center" width="48">인원</td>
									<td class="tdhead_center" width="88">절감금액</td>
									<td class="tdhead_center" width="80">수수료</td>
									<td class="tdhead_center" width="80">잔금</td>
									<td class="tdhead_center" width="48">분납</td>
...
for($i=0; $row=sql_fetch_array($result); $i++) {
  ...
  $list = $i%2;
	...
	//등록일자
	$date1 = substr($row['si4n_nhis_regdt'],0,10); //날짜표시형식변경
	//echo $date1;
	$si4n_nhis_regdt_time = substr($row['si4n_nhis_regdt'],11,8); //시간만 표시
	$date = explode("-", $date1); 
	$year = $date[0];
	$month = $date[1];
	$day = $date[2];
	...
	//급여대장 / 급여대장(샘플) 파일 등록 시로 변경 김국진 과장 의견 161025
