...
function goInsert() {
	...
	frm.action = "client_memo_iframe_update.php";
	frm.submit();
	return;
}
function memo_del(id,idx) {
	if(confirm("삭제하시겠습니까?")) {
		location.href = "client_memo_iframe_delete.php?id="+id+"&idx="+idx;
	}
}
//사업게시일 입력 콤마
...
<?
//덧글
$sql_comment = " select * from com_list_gy_memo where com_code='$row[com_code]' and delete_yn != '1' ";
...
for ($i=0; $row_comment=sql_fetch_array($result_comment); $i++) {
	...
	if($row_comment['visit']) $comment .= "<b>완료</b><input type='checkbox' name='ok_day' value='".$row_comment['ok_day']."' style='vertical-align:middle;'><b>재방문일</b>(<b style='color:blue'>".$row_comment['visit']."</b>) ";
	if($row_comment['call_day']) $comment .= "<b>재연락일</b>(<b style='color:blue'>".$row_comment['call_day']."</b>) ";
