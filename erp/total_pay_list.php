...
//사무위탁수임 : 초기선택 수임 -> 전체 150226 -> 수임 160212 -> 전체 160309
...
if($stx_total_process) {
	...
	//미신고
	else if($stx_total_process == "n") {
		//2015년 이상일 경우 처리완료 4 설정
		if($stx_total_year >= 2015) $sql_search .= " (d.ok_report != '4' and d.input_date != '') ";
		else $sql_search .= " (d.ok_report != '2' and d.input_date != '') ";
...
function total_input_func(obj) {
	var frm = document.searchForm;
	if(frm.stx_total_input_all.checked) {
		if(obj.name == "stx_total_input_all") {
			for(i=1; i<=6; i++) {
				frm['stx_total_input'+i].checked = true;
			}
		} else {
			frm['stx_total_input_all'].checked = false;
		}
	} else if(obj.name == "stx_total_input_all") {
		for(i=1; i<=6; i++) {
			frm['stx_total_input'+i].checked = false;
		}
	}
}
</script>
			...
			<div style="margin:10px 20px 20px 20px;min-height:480px;">
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						...
						<td width=""><a href="<?=$_SERVER['PHP_SELF']?>"><img src="images/top05_03.gif" border="0"></a></td>
						<td>
<?
$title_main_no = "05";
...
<? } ?>
									...
									<tr>
										...
										<td nowrap class="tdrow" colspan="3">
											<!--<b>년도</b>
											<select name="stx_total_year" class="selectfm" onChange="">
												<option value="">전체</option>
<?
for($i=2013;$i<=$total_year_fix;$i++) {
?>
												<option value="<?=$i?>" <? if($i == $stx_total_year) echo "selected"; ?> ><?=$i?></option>
<?
}
if($stx_total_input_all) $stx_total_input_all_chk = "checked";
else $stx_total_input_all_chk = "";
?>
											</select>-->
<input type="checkbox" name="stx_total_input_all" onclick="total_input_func(this)" value="1" <?=$stx_total_input_all_chk?> style="vertical-align:middle"><b>전체</b>
<?
if($stx_total_input1) $stx_total_input1_chk = "checked";
else $stx_total_input1_chk = "";
if($stx_total_input2) $stx_total_input2_chk = "checked";
else $stx_total_input2_chk = "";
if($stx_total_input3) $stx_total_input3_chk = "checked";
else $stx_total_input3_chk = "";
if($stx_total_input4) $stx_total_input4_chk = "checked";
else $stx_total_input4_chk = "";
if($stx_total_input5) $stx_total_input5_chk = "checked";
else $stx_total_input5_chk = "";
if($stx_total_input6) $stx_total_input6_chk = "checked";
else $stx_total_input6_chk = "";
?>
											<input type="checkbox" name="stx_total_input1" value="1" <?=$stx_total_input1_chk?> style="vertical-align:middle">이지노무
											<input type="checkbox" name="stx_total_input2" value="1" <?=$stx_total_input2_chk?> style="vertical-align:middle">키즈노무
											<input type="checkbox" name="stx_total_input3" value="1" <?=$stx_total_input3_chk?> style="vertical-align:middle">더존
											<input type="checkbox" name="stx_total_input4" value="1" <?=$stx_total_input4_chk?> style="vertical-align:middle">팩스
											<input type="checkbox" name="stx_total_input5" value="1" <?=$stx_total_input5_chk?> style="vertical-align:middle">메일
											<input type="checkbox" name="stx_total_input6" value="1" <?=$stx_total_input6_chk?> style="vertical-align:middle">기타
										</td>
...
?>
											<select name="stx_total_process" class="selectfm" onchange="">
												...
												<option value="4" <? if($total_process_chk == "4") echo "selected"; ?>>처리완료</option>
										...
										<td nowrap class="tdrowk"><img src="images/icon_02.gif" width="2" height="2" style="margin:0 5px 3px 0">건강신고</td>
										<td nowrap class="tdrow" colspan="">
											<select name="stx_gg_process" class="selectfm" onchange="">
												<option value=""  <? if($stx_gg_process == "")  echo "selected"; ?>>전체</option>
												<option value="r" <? if($stx_gg_process == "r") echo "selected"; ?>>접수중</option>
												<option value="1" <? if($stx_gg_process == "1") echo "selected"; ?>>처리중</option>
												<option value="2" <? if($stx_gg_process == "2") echo "selected"; ?>>처리완료</option>
												<option value="3" <? if($stx_gg_process == "3") echo "selected"; ?>>자체신고</option>
												<option value="4" <? if($stx_gg_process == "4") echo "selected"; ?>>반려</option>
												<option value="5" <? if($stx_gg_process == "5") echo "selected"; ?>>기타</option>
												<option value="n" <? if($stx_gg_process == "n") echo "selected"; ?>>미신고</option>
											</select>
										</td>
										<td nowrap class="tdrowk"><img src="images/icon_02.gif" width="2" height="2" style="margin:0 5px 3px 0">건강EDI위임</td>
										<td nowrap class="tdrow" colspan="">
								...
								<table width="100%" border="0" cellpadding="0" cellspacing="0" style="table-layout:fixed">
									<tr>
										<td align="center">
											...
											<a href="total_pay_excel.php?<?=$qstr?>" target=""><img src="./images/btn_excel_print_big.png" border="0"></a>
										</td>
									</tr>
								</table>
							</form>
							...
							<form name="dataForm" method="post">
								...
								<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="">
									<tr>
										...
										<td class="tdhead_center" width="80" rowspan="1">접수(<?=$total_year_fix-1?>)</td>
										<td class="tdhead_center" width="80" rowspan="1">접수(<?=$total_year_fix?>)</td>
										<td class="tdhead_center" width="80" rowspan="1">신고(고/산)</td>
									...
									<tr>
										...
										<td class="tdhead_center" width="" rowspan="1">신고(<?=$total_year_fix-1?>)</td>
										<td class="tdhead_center" width="" rowspan="1">신고(<?=$total_year_fix?>)</td>
										<td class="tdhead_center" width="" rowspan="1">건강신고</td>
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
  ...
  $list = $i%2;
	...
	//보수총액 신고현황 배열(2015년) 160203
	$total_req_yn_array_2014 = Array("","처리중","신고완료","자체신고","반려","기타");
	$total_req_yn_array = Array("","안내완료(1차)","안내완료(2차)","처리중","처리완료","자체신고","반려","해지예정","해지완료");
	//건강보험 신고현황
	$total_req_gg_array = Array("","처리중","처리완료","자체신고","반려","기타");
	//접수자료 2013 ~
	for($total_year=2014; $total_year<=$total_year_fix; $total_year++) {
		...
		if($row_total['ok_report_gg']) $total_req_gg_text[$total_year] = $total_req_gg_array[$total_req_gg_code];
		else $total_req_gg_text[$total_year] = "-";
	}
	//이지노무 키즈노무 구분 160212
	if($row_total['upjong_total']) {
		$upjong_total = " <span style='color:blue'>[".$row_total['upjong_total']."]</span>";
	} else {
		$upjong_total = "";
	}
	//소멸 사업장
	...
	if($member['mb_level'] > 6 || $row['damdang_code'] == $member['mb_profile']) {
		$com_view = "total_pay_view.php?id=$id&w=u&$qstr&page=$page";
	} else {
...
?>
									<tr class="<?=$tr_class?>" onMouseOver="this.className='list_row_over';" onMouseOut="this.className='<?=$tr_class?>';">
										...
										<td class="ltrow1_left_h22" title="">
											<a href="<?=$com_view?>" style="font-weight:bold"><?=$com_name_full?><?=$samu_close_text?><?=$samu_termination?><?=$upjong_total?></a>
										...
										<td class="ltrow1_center_h22"><span style="font-size:8pt;"><?=$total_input1[$total_year_fix-1]?><?=$total_input2[$total_year_fix-1]?><?=$total_input3[$total_year_fix-1]?><?=$total_input4[$total_year_fix-1]?><?=$total_input5[$total_year_fix-1]?><?=$total_input6[$total_year_fix-1]?></span><br><b><?=$total_req_yn_text[$total_year_fix-1]?></b></td>
										<td class="ltrow1_center_h22"><span style="font-size:8pt;"><?=$total_input1[$total_year_fix]?><?=$total_input2[$total_year_fix]?><?=$total_input3[$total_year_fix]?><?=$total_input4[$total_year_fix]?><?=$total_input5[$total_year_fix]?><?=$total_input6[$total_year_fix]?></span><br><b><?=$total_req_yn_text[$total_year_fix]?></b></td>
										<td class="ltrow1_center_h22"><?=$total_ok_date[$total_year_fix]?><br><b><?=$total_req_gg_text[$total_year_fix]?></b></td>
