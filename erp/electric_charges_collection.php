...
//필수 검색 분납 업체만 표시 160614
$sql_search .= " and a.electric_charges_installment != '' ";
//필수 검색 처리현황 최종완료(9) 제외 160614
//$sql_search .= " and a.electric_charges_process != '9' ";
//처리현황 수금진행만 표시 160909
$sql_search .= " and a.electric_charges_process = '14' ";
//사업장명칭
if ($stx_comp_name) {
	$sql_search .= " and ( ";
...
$qstr .= "&stx_electric_charges_visit_kind=".$stx_electric_charges_visit_kind."&stx_payments=".$stx_payments."&stx_cost=".$stx_cost."&stx_electric_charges_watt1=".$stx_electric_charges_watt1."&stx_electric_charges_watt2=".$stx_electric_charges_watt2;
?>
<html>
<head>
...
if($member['mb_level'] > 7) {
	//echo $stx_man_cust_name;
?>
									<td nowrap class="tdrowk" width="70"><img src="images/icon_02.gif" width="2" height="2" style="margin:0 5 3 0">문자</td>
									<td nowrap class="tdrow" width="98">
										<table border="0" cellpadding="0" cellspacing="0" style="float:left;vertical-align:middle;margin-left:0;"><tr><td width="2"></td><td><img src="images/btn_lt.gif"></td><td background="images/btn_bg.gif" class="ftbutton1" nowrap><a href="electric_charges_sms.php">고객관리</a></td><td><img src="images/btn_rt.gif"></td><td width="2"></td></tr></table>
									</td>
<? } else { ?>
									<td nowrap class="tdrowk"></td>
									<td nowrap class="tdrow">
...
<? } ?>
						...
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<tr>
								<td align="left" width="290" style="font-weight:bold;">
									※ 처리현황 수금진행, 분납 거래처 현황입니다.
								</td>
								<td align="center">
						...
						<form name="dataForm" method="post">
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="">
								<tr>
									...
									<td class="tdhead_center" rowspan="2" width="">사업장명</td>
									<td class="tdhead_center" rowspan="2" width="84">고객번호<br />청구일</td>
									<td class="tdhead_center" rowspan="2" width="54">조회</td>
									<td class="tdhead_center" rowspan="2" width="70">첫수금일</td>
									<td class="tdhead_center" rowspan="2" width="54">분납</td>
									<td class="tdhead_center" colspan="<?=$collection_month?>">수수료 분납</td>
									<td class="tdhead_center" rowspan="2" width="110">담당자</td>
								</tr>
									<tr>
<?
for($m=1; $m<=$collection_month; $m++) {
?>
										<td class="tdhead_center" width="20"><?=$m?></td>
<?
}
?>
									</tr>
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
  ...
  $list = $i%2;
	...
	//분납 회차 / 초기 idx 추출, 전정애 대리 건의 160929
	$sql_app = " select * from erp_application where com_code='$id' and application_kind='23' order by idx asc limit 0, 1 ";
	$row_app = sql_fetch($sql_app);
	$idx = $row_app['idx'];
	//계약금
	if($row_app['down_payment']) $down_payment = number_format($row_app['down_payment']);
	else $down_payment = "-";
	//전체잔금
	if($row_app['remainder']) $remainder = number_format($row_app['remainder']);
	else $remainder = "-";
	//분납DB
	$sql_account = " select remainder_date, remainder_vat, requested_amount from erp_application_surplus where mid='$idx' order by sid asc ";
	//echo $sql_account." ".$id."<br />";
	//분납 초기화
	for($m=1;$m<=24;$m++) {
		$account[$m] = "";
		$account_date[$m] = "";
	...
	for($m=1; $row_account=sql_fetch_array($result_account); $m++) {
		if($row_account['remainder_date']) {
			$account[$m] = "O";
			$account_date[$m] = $row_account['remainder_date']."&#13;";
			//$account_vat[$m] = number_format($row_account['remainder_vat']);
			//절감금액 -> 청구금액 표시, 전정애 대리 요청 161201
			$account_vat[$m] = number_format($row_account['requested_amount']);
		} else {
			$account[$m] = "";
			$account_date[$m] = "";
			$account_vat[$m] = "";
			//청구금액만 있는 경우 160909
			if($row_account['requested_amount']) {
				//공사비 161027
				if($row_account['requested_amount'] == "공사비") {
					$account[$m] = "ⓒ";
					$account_date[$m] = "공사비";
				} else {
					$account[$m] = "-";
	...
	if($row['electric_date_request']) {
		...
		//7일에서 9일로 변경 임현미 의견 160510 -> 10일 160926
	...
	//첫수금일 = 스케줄관리(본사) 160907
	if($row['electric_charges_visitdt2']) $electric_charges_visitdt2 = $row['electric_charges_visitdt2'];
	else $electric_charges_visitdt2 = "";
?>
								<tr class="list_row_now_wh" onMouseOver="this.className='list_row_over';" onMouseOut="this.className='list_row_now_wh';">
									...
									<td class="ltrow1_left_h22" title="<?=$com_name_full?>">
										<a href="electric_charges_collection_view.php?id=<?=$id?>&w=u&<?=$qstr?>&page=<?=$page?>" style="font-weight:bold"><?=$com_name?></a>
										<br><?=$com_juso?>
									</td>
									<td class="ltrow1_center_h22"><?=$electric_charges_no?><br /><?=$row['electric_date_request']?></td>
									<td class="ltrow1_center_h22"><?=$electric_date_inquiry_day?></td>
									<td class="ltrow1_center_h22"><?=$electric_charges_visitdt2?></td>
									<td class="ltrow1_center_h22"><?=$electric_charges_installment?></td>
<?
	for($m=1; $m<=$collection_month; $m++) {
?>
									<td nowrap class="ltrow1_center_h22" title="<?=$account_date[$m]?><?=$account_vat[$m]?>"><?=$account[$m]?></td>
<?
	}
	$sel_check_ok = array();
