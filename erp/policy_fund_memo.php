...
//사업장DB 옵션
$sql1 = " select * from policy_fund where id='$id' ";
//echo $sql1;
...
function goInsert() {
	...
	frm.action = "policy_fund_memo_update.php";
	frm.submit();
	return;
}
</script>
<div style="padding:0 0 5px 0;">
	<b>상호: <?=$row[com_name]?> ·업종: <?=$row[upjong]?> ·핸드폰: <?=$row[com_hp]?> ·지역: <?=$row[area]?></b>
</div>
<?=$row[memo]?>
(<?=$row[regdt]?>)
...
<?
//덧글
$sql_comment = " select * from policy_fund_comment where mid='$row[id]' and delete_yn != '1' ";
...
for ($i=0; $row_comment=sql_fetch_array($result_comment); $i++) {
	...
	$comment_del = " <a href='policy_fund_memo_delete.php?id=".$id."&idx=".$row_comment['idx']."'><img src='images/co_btn_delete.gif' border='0' style='vertical-align:middle;'></a>";
