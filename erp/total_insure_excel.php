...
$cell = array("등록No","위탁No","등록일자","부과고지","관리공단","사업장명","우편번호","주소","상세주소","사업자등록번호","사업장관리번호","전화번호","팩스번호","이메일","대표자","사업개시일","업태","업종","위탁서접수","수임일자","해지일자","처리현황","건강EDI","고용","산재","관리점","담당자","TM매니저","접수(".$total_req_year.")","신고(".$total_req_year.")","신고일자");

$now_date_file = date("Ymd");
$file_name = "보험료신고_".$now_date_file.".xls";
header("Pragma:");
...
?>
			<tr bgcolor="<?=$tr_color?>" align=center>
				...
				<td align="center"><?=$total_input1[$total_req_year]?><?=$total_input2[$total_req_year]?><?=$total_input3[$total_req_year]?><?=$total_input4[2015]?><?=$total_input5[2015]?><?=$total_input6[$total_req_year]?></td>
				<td align="center"><?=$total_req_yn_text[$total_req_year]?></td>
				<td align="center"><?=$total_ok_date[$total_req_year]?></td>
			</tr>
<?
}
if ($i == 0)
