...
function goInput(mode){
	...
	*/
	...
	for(i=1;i<=<?=$total_count?>;i++) {
		...
		f["e9_"+k].value = f.money_e9[i].value;
														...
														<td>  
															<table width="<?=$pay_list_width?>" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:;">
																...
																<tr>
																	...
																	<td class="tdhead_center" rowspan="2" width="71">컨설팅<br />수당</td>
...
?>
																...
																<tr>
																	...
																	<td class="tdhead_center" width="109"><input type="text" name="g1" class="textfm5" readonly  style="width:100%;" value="<?=$money_g_txt['g1']?>"></td>
																	<td class="tdhead_center" width="109"><input type="text" name="g2" class="textfm5" readonly  style="width:100%;" value="<?=$money_g_txt['g2']?>"></td>
																	<td class="tdhead_center" width="109"><input type="text" name="g3" class="textfm5" readonly  style="width:100%;" value="<?=$money_g_txt['g3']?>"></td>
																	<td class="tdhead_center" width="109"><input type="text" name="g4" class="textfm5" readonly  style="width:100%;" value="<?=$money_g_txt['g4']?>"></td>
																	<td class="tdhead_center" width="109"><input type="text" name="g5" class="textfm5" readonly  style="width:100%;" value="<?=$money_g_txt['g5']?>"></td>

																	<td class="tdhead_center" width="76"><input type="text" name="b1" class="textfm5" readonly  style="width:100%;" value="<?=$money_e_txt['e1']?>"></td>
																	<td class="tdhead_center" width="76"><input type="text" name="b2" class="textfm5" readonly  style="width:100%;" value="<?=$money_e_txt['e2']?>"></td>
																	<td class="tdhead_center" width="76"><input type="text" name="b3" class="textfm5" readonly  style="width:100%;" value="<?=$money_e_txt['e3']?>"></td>
																	<td class="tdhead_center" width="76"><input type="text" name="b4" class="textfm5" readonly  style="width:100%;" value="<?=$money_e_txt['e4']?>"></td>
																	<td class="tdhead_center" width="76"><input type="text" name="b5" class="textfm5" readonly  style="width:100%;" value="<?=$money_e_txt['e5']?>"></td>
																	<td class="tdhead_center" width="76"><input type="text" name="b6" class="textfm5" readonly  style="width:100%;" value="<?=$money_e_txt['e6']?>"></td>
																	<td class="tdhead_center" width="76"><input type="text" name="b7" class="textfm5" readonly  style="width:100%;" value="<?=$money_e_txt['e7']?>"></td>
																	<td class="tdhead_center" width="76"><input type="text" name="b8" class="textfm5" readonly  style="width:100%;" value="<?=$money_e_txt['e8']?>"></td>
																	<td class="tdhead_center" width="76"><input type="text" name="b9" class="textfm5" readonly  style="width:100%;" value="<?=$money_e_txt['e9']?>"></td>
																</tr>
															</table>
									...
									<tr>
										<td width="200" bgcolor="#f7fafd" valign="top">
<?
											...
											<div id="spanMain" style="width:100%;height:<?=$spanMain_height?>px;overflow-x:hidden;overflow-y:auto;" onScroll="spanLeft.scrollTop = this.scrollTop; spanTop.scrollLeft = this.scrollLeft;">
												<table width="<?=$pay_list_width?>" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:fixed;">
													...
													for ($i=0; $row=sql_fetch_array($result); $i++) {
														...
														//부서
														if($row2['dept_1']) {
															$sql_dept = " select * from com_code_list where item='dept' and com_code='$row[com_code]' and code = $row2[dept_1] ";
...
if($row4['money_month'] == 0 || $data == "load") {
	...
	for($j=0; $row_attendance=sql_fetch_array($result_attendance); $j++) {
		...
		//echo $att_date[0];
		//if($row[sabun] == "0004") echo $att_day;
		//해당 년도
		//if($att_date[0] == $search_year && $att_date[1] == $search_month) {
		//임금산출기간
		//echo $att_day."/ ";
		if( ($att_date[0] == $pay_calculate_year1 && $att_date[1] == $pay_calculate_month1 && $att_date[2]>=$row_com_opt['pay_calculate_day_period1'] && $att_date[2]<=31) || ($att_date[0] == $pay_calculate_year2 && $att_date[1] == $pay_calculate_month2 && $att_date[2]>=1 && $att_date[2]<=$row_com_opt['pay_calculate_day_period2']) ) {
			...
			//if($row[sabun] == "0004") echo $att_category." ".($att_rule[$att_category]-1)." ";
			...
			//휴일근로(사원정보 + 근태등록)
			else if($att_category == 11) $workhour_hday += $att_rule[$att_category] - 1;
		}
	}
	if($j == 0) {
		$w_late = "";
	...
	$money_e9 = $row3[money_e9];

	$etc = $row3[money_etc];
	//사원 급여정보 4대보험 값 호출 160401
	$money_yun = $row3['money_yun'];
	$money_health = $row3['money_health'];
	$money_yoyang = $row3['money_yoyang'];
	$money_goyong = $row3['money_goyong'];
} else {
	if($pay_gbn_no == 1) {
	...
	$money_yun = $row4[yun];
	$money_health = $row4[health];
	$money_yoyang = $row4[yoyang];
	$money_goyong = $row4[goyong];
}
//최저시급 DB 출력
