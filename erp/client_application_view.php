...
function main_receipt_fee_cal(k) {
	...
	//본사수입 160829
	main_income = requested_amount;
	frm['main_income'+k].value = number_format(main_income);
}
//신청금액 = 입금금액 복사
...
//본사입금금액 계산
function requested_amount_cal(k) {
	var frm = document.dataForm;
	requested_amount = toInt(frm['requested_amount'+k].value);
	if(requested_amount == "") {
...
<?
include "inc/top.php";
$url_list = "./client_application_list.php?page=".$page;
?>
			...
			<div style="margin:10px 20px 20px 20px;min-height:480px;">
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
					<tr>
						...
						<td width=""><a href="<?=$url_list?>"><img src="images/top02_01.gif" border="0" /></a></td>
						<td></td>
					</tr>
...
//전기,물, 농지, 취등록세
if($application_kind[$app_no] >= 11 && $application_kind[$app_no] <= 13) $fee_kind = "contribution";
...
?>
											...
											<td nowrap class="tdrowk" width="70"><img src="./images/icon_02.gif" width="2" height="2" style="margin:0 5px 3px 0">접수일자</td>
...
?>
											...
											<td nowrap class="tdrowk">
												<img src="./images/icon_02.gif" width="2" height="2" style="margin:0 5px 3px 0">본사수입
											</td>
											<td nowrap class="tdrow">
<?
if($row2['main_income']) $main_income = number_format($row2['main_income']);
else $main_income = "";
if($member['mb_level'] != 6) {
?>
												<input name="main_income<?=$k?>" type="text" class="textfm" style="width:120px;ime-mode:disabled;" value="<?=$main_income?>" onKeyPress="only_number()" onkeyup="checkThousand(this.value, this,'Y');">
<?
} else {
...
?>
											...
											<td nowrap class="tdrow">
												<input type="checkbox" name="certification_content<?=$k?>" <? if($row2['certification_content']) echo "checked"; ?> value="1" style="vertical-align:middle">내용증명
												<input type="checkbox" name="part_receipt<?=$k?>" <? if($row2['part_receipt']) echo "checked"; ?> value="1" style="vertical-align:middle">부분입금
											</td>
										</tr>
										<tr>
...
if($member['mb_level'] >= 8) {
	$url_save = "javascript:checkData('client_application_update.php');";
} else {
