...
$html_title = "4대보험절감계산기(시급)";
$colspan = 12;
//$total_count = 10;
$total_count = $_GET['cnt'];
//테이블 넓이			기본	절감액	시급+시간	사업장4대보험 산재보험
$pay_list_width = 837 + 74 		+ 88 + 			488          + (58*2);
?>
<html>
<head>
...
function focusInClass(idx) {
	...
	frm.time[idx].className = "textfm_trans";
...
function goInput(mode){
	...
	for(i=1;i<=<?=$total_count?>;i++) {
		...
		f["time_"+k].value = f.time[i].value;
	...
	f.mode.value = mode;
	f.action = "pop_si4n_nhis_calculate3_update.php";
	f.submit();
}
//절감금액 입력
function data_input() {
	...
	opener.document.dataForm.si4n_pay_month2.value = sum_money_result_val;
	var sum_money_result_num = ('' + sum_money_result_val).replace(/,/g, "");
	//alert(sum_money_result_num);
	opener.document.dataForm.si4n_pay_year2.value = number_format(sum_money_result_num * 12);
	return;
}
//숫자만 입력 ('-' 포함)
...
function del(id) {
	if(id) {
		var f = document.dataForm;
		f.action = "pop_si4n_nhis_calculate3_del.php";
									...
									<tr>
										<td>  
											<table width="<?=$pay_list_width?>" border="0" cellpadding="0" cellspacing="1" class="bgtable">
												<tr>
													<td class="tdhead_center" colspan="5">급여정보</td>
												...
												<tr>
													...
													<td class="tdhead_center" rowspan="2" width="58">시급</td>
													<td class="tdhead_center" rowspan="2" width="30">시간</td>
...
for ($i=0; $i<$total_count; $i++) {
	...
	//else $sabun = $k;
	//4대보험절감 급여DB
	$sql = " select * from si4n_nhis_pay_time where com_code='$id' and sabun='$k' ";
	$result = sql_query($sql);
	$row = mysql_fetch_array($result);
	if($row['family_cnt']) $family_cnt = $row['family_cnt'];
	else $family_cnt = 1;
	$money_time = $row['money_time'];
	$time = $row['time'];
...
?>
									<tr class="list_row_now_gr" onMouseOver="this.className='list_row_over';" onMouseOut="this.className='list_row_now_gr';">
										...
										<input type="hidden" name="time_<?=$i?>" />
...
?>
										...
										<td nowrap class="ltrow1_center_h24" width="58"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="money_time" id="id_money_time_<?=$k?>" value="<?if($money_time) echo number_format($money_time);?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay('<?=$k?>');" onkeydown="if(event.keyCode == 13){ getId('id_money_time_<?=$k_next?>').select(); }" /></td><!-- 시급 -->
										<td nowrap class="ltrow1_center_h24" width="30"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="time" id="id_time_<?=$k?>" value="<?if($time) echo number_format($time);?>" onKeyUp="cal_pay('<?=$k?>');" onkeydown="if(event.keyCode == 13){ getId('id_time_<?=$k_next?>').select(); }" /></td><!-- 시간 -->

										<td nowrap class="ltrow1_center_h24" width="74"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="tax_exemption"  id="id_tax_exemption_<?=$k?>"  value="<?if($row['tax_exemption']) echo number_format($row['tax_exemption'])?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay('<?=$k?>');" onkeydown="if(event.keyCode == 13){ getId('id_tax_exemption_<?=$k_next?>').select(); }" /></td><!-- 절감액 -->

										<td nowrap class="ltrow1_center_h24" width="74"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="money2_for_tax" id="id_money2_for_tax_<?=$k?>" value="<?=number_format($row['money2_for_tax'])?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay('<?=$k?>');" onkeydown="if(event.keyCode == 13){ getId('id_money2_for_tax_<?=$k_next?>').select(); }" /></td><!-- 변경후 -->
										...
										<td nowrap class="ltrow1_center_h24" width="58"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="money2_yun" id="id_money2_yun_<?=$k?>" value="<?=number_format($money2_yun)?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay2('<?=$k?>');" onkeydown="if(event.keyCode == 13){ getId('id_money_yun_<?=$k_next?>').select(); }" /></td><!-- 국민연금 -->
										<td nowrap class="ltrow1_center_h24" width="58"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="money2_health" id="id_money2_health_<?=$k?>" value="<?=number_format($money2_health)?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay2('<?=$k?>');" onkeydown="if(event.keyCode == 13){ getId('id_money_health_<?=$k_next?>').select(); }" /></td><!-- 건강보험 -->
										<td nowrap class="ltrow1_center_h24" width="58"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="money2_yoyang" id="id_money2_yoyang_<?=$k?>" value="<?=number_format($money2_yoyang)?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay2('<?=$k?>');" onkeydown="if(event.keyCode == 13){ getId('id_money_yoyang_<?=$k_next?>').select(); }" /></td><!-- 장기요양보험 -->
										<td nowrap class="ltrow1_center_h24" width="58"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="money2_goyong" id="id_money2_goyong_<?=$k?>" value="<?=number_format($money2_goyong)?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay2('<?=$k?>');" onkeydown="if(event.keyCode == 13){ getId('id_money_goyong_<?=$k_next?>').select(); }" /></td><!-- 고용보험 -->
						...
						<td nowrap class="tdhead_center" valign="top">
							<table width="<?=$pay_list_width?>" border="0" cellpadding="0" cellspacing="1" class="bgtable">
								<tr class="list_row_now_wh">
									...
									<td nowrap class="ltrow1_center_h24" width="58"><span id="sum_money_time"><?=number_format($row['sum_money_time'])?></span></td>
									<td nowrap class="ltrow1_center_h24" width="30"><!--<span id="sum_time"><?=number_format($row['sum_time'])?></span>--></td>
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" style="table-layput:fixed">
				<tr>
					<td align="center">
						...
						<table border="0" cellpadding="0" cellspacing="0" style="display:inline;margin-right:20px;"><tr><td width="2"></td><td><img src="images/btn_lt.gif"></td><td background="images/btn_bg.gif" class="ftbutton1" nowrap><a href="pop_si4n_nhis_excel3.php?id=<?=$id?>">엑 셀</a></td><td><img src="images/btn_rt.gif"></td><td width="2"></td></tr></table>
						<table border="0" cellpadding="0" cellspacing="0" style="display:inline;margin-right:20px;"><tr><td width="2"></td><td><img src="images/btn_lt.gif"></td><td background="images/btn_bg.gif" class="ftbutton1" nowrap><a href="pop_si4n_nhis_excel3_result.php?id=<?=$id?>">절감결과</a></td><td><img src="images/btn_rt.gif"></td><td width="2"></td></tr></table>
...
?>
	...
	money_time = toInt(f.money_time[<?=$i?>].value);
	time = toInt(f.time[<?=$i?>].value);
...
?>
	...
	$('#sum_money_time').html(number_format(sum_money_time));
	//$('#sum_time').html(number_format(sum_time));
...
function cal_pay(idx) {
	...
	money_time   = toInt(f.money_time [idx].value);	//시급
	time   = toInt(f.time [idx].value);	//시간
	...
	//비과세 적용 금액(시급) 161017 / 김국진 과장 의견 1.5배
	//tax_exemption = (money_time * time) * 1.5;
	//절감액 수동 입력 기능 161026
	if(money_time) tax_exemption = (money_time * time) * 1.5;
	money2_for_tax = money_for_tax - tax_exemption;
...
?>
	...
	//시급 입력 시 절감액 계산 161104
	if(money_time) {
		//alert(money_time);
		tax_exemption = number_format(tax_exemption);
		//절감액 계산 161017
		f.tax_exemption[idx].value = tax_exemption;
	}
	//변경 후
