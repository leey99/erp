...
//DB 필드, memo(사유), v_code(휴가 종류), work_forenoon(시작일), work_afternoon(종료일), work_night(긴급연락망)
$sql_common = " regdt = '$now_time',
						...
						work_forenoon = '$v_start_date',
						work_afternoon = '$v_end_date',
						work_plan = '$v_place',
						work_night = '$v_tel',
...
if($report_bt) {
	...
	$sql_alert = " insert erp_alert set 
			...
			alert_code = '90014', alert_name = '휴가신청'
	";
	//echo $sql_alert;
	//exit;
	sql_query($sql_alert);
	alert("정상적으로 휴가신청서가 상신 되었습니다.","groupware_business_log.php?stx_process=all");
} else {
	if ($w == 'u'){
	...
	alert("정상적으로 휴가신청서가 임시저장 되었습니다.","groupware_business_log.php?stx_process=all");
}
?>
