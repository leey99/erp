...
//필수 검색조건 직무발명보상제도 의뢰 체크 160824 / 출원인번호 160926
//$sql_search .= " and c.job_invent_recompense = 1 ";
$sql_search .= " and c.job_invent_no != '' ";
//사업장명칭
if ($stx_comp_name) {
	$sql_search .= " and ( ";
...
//출원인 번호
if($stx_job_invent_no) {
	$sql_search .= " and ( ";
	$sql_search .= " (c.job_invent_no = '$stx_job_invent_no') ";
	$sql_search .= " ) ";
}
//업종분류검색
...
if($stx_process) {
	$sql_search2 = " and ( ";
	if($stx_process == "no") $sql_search2 .= " (c.job_invent_process = '') ";
	else $sql_search2 .= " (c.job_invent_process = '$stx_process') ";
	$sql_search2 .= " ) ";
}
//분납 160614
...
if($stx_search_day_chk) {
	...
	$sql_search .= " and (c.job_invent_regdt >= '$search_sday_var' and c.job_invent_regdt <= '$search_eday_var') ";
}
//정렬
if (!$sst) {
    $sst = "c.job_invent_regdt";
    $sod = "desc";
}
		...
		<td onmouseover="showM('900')" valign="top" style="padding:10px 20px 20px 20px;min-height:480px;">
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td><a href="<?=$php_self_list?>"><img src="<?=$top_sub_title?>" border="0" alt="전기요금컨설팅" /></a></td>
					<td>
<?
$title_main_no = "19";
//딜러 제외한 모두 표시
...
<? } ?>
								...
								<tr>
									<td nowrap class="tdrowk"><img src="images/icon_02.gif" width="2" height="2" style="margin:0 5px 3px 0;">출원인번호</td>
									<td nowrap class="tdrow" colspan="">
										<input name="stx_job_invent_no"  type="text" class="textfm" style="width:90px;ime-mode:disabled;" value="<?=$stx_job_invent_no?>" maxlength="12"  onkeydown="if(event.keyCode == 13){ goSearch(); }">
									</td>
...
$collet_money = 0;
//검색 쿼리 초기화
$sql_search_cnt = " where a.com_code = b.com_code and a.com_code = c.com_code and c.job_invent_no != '' ";
//본사, 제휴점, 지사 구분 검색
...
$sql_job_invent = " select c.job_invent_process from com_list_gy a, com_list_gy_opt b, com_list_gy_opt2 c ";
$sql_job_invent .= " $sql_search_cnt $sql_search_add $sql_search_add2 ";
//echo $sql_job_invent;
$result_job_invent = sql_query($sql_job_invent);
for ($i=0; $row_job_invent=mysql_fetch_assoc($result_job_invent); $i++) {
	if($row_job_invent['job_invent_process'] == 1) $contracts_progress++;
	else if($row_job_invent['job_invent_process'] == 2) $progress_task++;
	else if($row_job_invent['job_invent_process'] == 3) $progress_wrong++;
	else if($row_job_invent['job_invent_process'] == 4) $reserve++;
	else if($row_job_invent['job_invent_process'] == 5) $counsel_complete++;
	else if($row_job_invent['job_invent_process'] == 6) $progress_cancel++;
	else if($row_job_invent['job_invent_process'] == 7) $collet_money++;
	else if($row_job_invent['job_invent_process'] == 8) $complete_task++;
	else if($row_job_invent['job_invent_process'] == "") $unselect++;
}
$job_total_count = $i;
?>
						<div>
...
?>
									...
									<td class="tdhead_center" width="70">대표자</td>
									<td class="tdhead_center" width="84">출원인번호</td>
									...
									<td class="tdhead_center" width="70">분납</td>
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
  ...
  $list = $i%2;
	$id = $row['com_code'];
	//등록일자
	$date1 = substr($row['job_invent_regdt'],0,10); //날짜표시형식변경
	//echo $date1;
	$job_invent_regdt_time = substr($row['job_invent_regdt'],11,8); //시간만 표시
	$date = explode("-", $date1); 
	$year = $date[0];
	$month = $date[1];
	$day = $date[2];
	$job_invent_regdt = $year.".".$month.".".$day."";
	//사업장명
	...
	//상담메모
	if($row['job_invent_memo']) $memo_full = $row['job_invent_memo'];
	...
	//출원인 번호
	if($row['job_invent_no']) {
		$job_invent_no = $row['job_invent_no'];
	} else {
		$job_invent_no = "-";
	}
	...
	//첨부서류
	if($row['job_invent_file_1']) $job_invent_file_1 = "○";
	else $job_invent_file_1 = "-";
	//계약금
	if($row['job_invent_down_payment']) $job_invent_down_payment = $row['job_invent_down_payment'];
	else $job_invent_down_payment = "-";
	//잔금
	if($row['job_invent_remainder']) $job_invent_remainder = $row['job_invent_remainder'];
	else {
		if($row['job_invent_remainder2']) $job_invent_remainder = $row['job_invent_remainder2'];
		else $job_invent_remainder = "-";
	}
	//절감예상금액
	...
	if($row['job_invent_fee']) $job_invent_fee = $row['job_invent_fee'];
		...
		if($row['job_invent_fee2']) $job_invent_fee = $row['job_invent_fee2'];
		else $job_invent_fee = "-";
	}
	//분납
...
?>
								<tr class="list_row_now_wh" onMouseOver="this.className='list_row_over';" onMouseOut="this.className='list_row_now_wh';">
									...
									<td class="ltrow1_center_h22" title="<?=$job_invent_regdt_time?>"><?=$job_invent_regdt?></td>
									<td class="ltrow1_left_h22" title="<?=$com_name_full?>">
										<a href="job_invent_recompense_view.php?id=<?=$id?>&w=u&<?=$qstr?>&page=<?=$page?>" style="font-weight:bold"><?=$com_name?></a>
										<br /><?=$com_juso?>
									</td>
									<td class="ltrow1_center_h22" title="<?=$boss_name_full?>"><?=$boss_name?></td>
									<td class="ltrow1_center_h22"><?=$job_invent_no?></td>
...
<? } ?>
									...
									<td class="ltrow1_center_h22"><?=$job_invent_file_1?></td>
									<td class="ltrow1_center_h22"><?=$job_invent_down_payment?></td>
									<td class="ltrow1_center_h22"><?=$job_invent_remainder?></td>
									<td class="ltrow1_center_h22"><?=$job_invent_fee?></td>
									<td class="ltrow1_center_h22"><?=$electric_charges_installment?></td>
<?
$sel_check_ok = array();
...
<?
	...
	if($job_invent_process_arry[$check_ok_id]) echo $job_invent_process_arry[$check_ok_id];
	else echo "-";
}
?>
									</td>
