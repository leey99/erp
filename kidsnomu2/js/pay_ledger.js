...
function OnStart(){
	...
	if(frm.comp_type.value=='D') {
		pay_table = 'pay_ledger_d.hwp';
	} else if(frm.comp_type.value=='E') {
		pay_table = 'pay_ledger_e.hwp';
	} else if(frm.comp_type.value=='F') {
		if(pay_count > 6 && pay_count < 13) pay_table = 'pay_ledger_f_2page.hwp';
		else pay_table = 'pay_ledger_f.hwp';
	} else {
		if(pay_count > 6 && pay_count < 13) pay_table = 'pay_ledger_2page.hwp';
		else if(pay_count > 12 && pay_count < 19) pay_table = 'pay_ledger_3page.hwp';
		else if(pay_count > 18) pay_table = 'pay_ledger_4page.hwp';
	...
	if(!pHwpCtrl.Open(BasePath + "/files/docs/"+pay_table)) {
		...
		for(i=0;i<=tr_count;i++){
			...
			if(frm.comp_type.value=='D' || frm.comp_type.value=='E') {
				if (pHwpCtrl.MoveToField("기타계"+i, true, true, false)){set.SetItem("Text",frm.b7[i].value);act.Execute(set);}
			}
		}
		//1page
		...
		if(frm.comp_type.value=='D' || frm.comp_type.value=='E') {
			if (pHwpCtrl.MoveToField("기타계s", true, true, false)){set.SetItem("Text",frm.b7_sum.value);act.Execute(set);}
		}
