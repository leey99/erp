...
function OnStart(){
	...
	pay_table = 'pay_ledger_s_'+pay_page+'page.hwp';
	...
	if(!pHwpCtrl.Open(BasePath + "/files/docs/"+pay_table)) {
		...
		for(k=0;k<pay_page;k++) {
			...
			if (pHwpCtrl.MoveToField("비과세s"+m, true, true, false)){set.SetItem("Text",frm.tax_exemption_sum[k].value);act.Execute(set);}
			if (pHwpCtrl.MoveToField("과세s"+m, true, true, false)){set.SetItem("Text",frm.taxation_sum[k].value);act.Execute(set);}
