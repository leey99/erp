...
function _GetBasePath(){
	...
	{
		return loc.substr(0,loc.lastIndexOf("/") + 1);
	}
	else // local
	{
		var path;
...
function OnStart(){
	...
	pay_table = 'pay_ledger_g_'+pay_page+'page.hwp';
	...
	if(!pHwpCtrl.Open(BasePath + "/files/docs/"+pay_table)) {
		...
		for(k=0;k<pay_page;k++) {
			...
			if (pHwpCtrl.MoveToField("etc1s"+m, true, true, false)){set.SetItem("Text",frm.etc_sum[k].value);act.Execute(set);}
			if (pHwpCtrl.MoveToField("etc2s"+m, true, true, false)){set.SetItem("Text",frm.etc2_sum[k].value);act.Execute(set);}
			...
			if (pHwpCtrl.MoveToField("세금계s"+m, true, true, false)){set.SetItem("Text",frm.tax_sum_sum[k].value);act.Execute(set);}

			if (pHwpCtrl.MoveToField("세금총액s"+m, true, true, false)){set.SetItem("Text",frm.tax_sum[k].value);act.Execute(set);}
