...
if($item == "position") {
	...
	$pay_type = "급수";
	$amount_name = "금액";
	$amount_onkeyup = "checkThousand(this.value, '1','Y')";
} else if($item == "step") {
	$sub_title = "호봉코드(급여연동)";
	$sub_menu = "100202";
	$code_name = "호봉명칭";
...
if($w == "u") {
	...
	if($item == "dept") {
		...
		if($row['amount']) $amount = number_format($row['amount']);
	}
}
?>
<head>
						...
						<td valign="top" style="padding:10px 10px 10px 10px">
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable1" style="table-layput:fixed">
								...
								<tr>
									...
									<td nowrap  class="tdrow">
										<input type="text" name="amount" class="textfm" style="width:150px;" value="<?=$row[amount]?>" maxlength="25" onkeyup="<?=$amount_onkeyup?>" />
									</td>
								</tr>
								<tr>
