...
//검색 : 버전
if($stx_lawwiz_code) {
	$sql_search .= " and (a.lawwiz_code = '$stx_lawwiz_code') ";
}
//검색 : 유/무료
if($stx_free_yn) {
	$sql_search .= " and ( ";
	if($stx_free_yn == 1) $sql_search .= " b.month_pay != 0 ";
	else $sql_search .= " b.month_pay = 0 ";
	$sql_search .= " ) ";
}
//검색 : 사무위탁
if($stx_samu_req_yn != "") {
...
?>
										...
										<td nowrap class="tdrowk"><img src="/img/ims/icon_02.gif" width="2" height="2" style="margin:0 5 3 0">버전</td>
										<td nowrap class="tdrow">
											<select name="stx_lawwiz_code" class="selectfm" onchange="goSearch();">
												<option value="" <? if($stx_lawwiz_code == "") echo "selected"; ?>>전체</option>
												<option value="1" <? if($stx_lawwiz_code == "1") echo "selected"; ?>>v1.0</option>
												<option value="2" <? if($stx_lawwiz_code == "2") echo "selected"; ?>>v2.0</option>
												<option value="f" <? if($stx_lawwiz_code == "f") echo "selected"; ?>>Lite</option>
											</select>
										</td>
										<td nowrap class="tdrowk"><img src="/img/ims/icon_02.gif" width="2" height="2" style="margin:0 5 3 0">유/무료</td>
										<td nowrap class="tdrow">
											<select name="stx_free_yn" class="selectfm" onchange="goSearch();">
												<option value=""  <? if($stx_free_yn == "")  echo "selected"; ?>>전체</option>
												<option value="1" <? if($stx_free_yn == "1") echo "selected"; ?>>유료</option>
												<option value="2" <? if($stx_free_yn == "2") echo "selected"; ?>>무료</option>
											</select>
										</td>
										...
										<td nowrap class="tdrow">
											<input name="stx_name" type="text" class="textfm" style="width:120;ime-mode:active;" value="<?=$stx_name?>" onkeydown="if(event.keyCode == 13){ goSearch(); }">
										</td>
										...
										<td nowrap class="tdrow">
											<select name="stx_upche_div" class="selectfm">
												...
												<option value="1" <? if($stx_upche_div == "1") echo "selected"; ?>>개인</option>
												<option value="2" <? if($stx_upche_div == "2") echo "selected"; ?>>법인</option>
											</select>
										</td>
...
<? } ?>
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:fixed;">
								<tr>
									...
									<td nowrap class="tdhead_center" width="42">버전</td>
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
  ...
  $list = $i%2;
	...
	//해지일 존재시 서비스기간에 해지일 표시
	if($row_opt[leave_day]) {
		$service_day = "해지 ".$row_opt[leave_day];
	}
	//세팅비
...
?>
								<tr class="list_row_now_wh" onMouseOver="this.className='list_row_over';" onMouseOut="this.className='list_row_now_wh';">
									...
									<td nowrap class="ltrow1_center_h22"><?=$lawwiz_code?></td>
