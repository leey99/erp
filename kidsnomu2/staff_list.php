...
$sql_order = " order by a.out_day asc , $sst $sod $sst2 $sod2 $sst3 $sod3 $sst4 $sod4 ";
							...
							<form name="searchForm" method="post">
								<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable1" style="table-layout:">
									<tr>
										...
										<td nowrap class="tdrowk"><img src="/img/ims/icon_02.gif" width="2" height="2" style="margin:0 5 3 0">채용형태</td>
										<td nowrap class="tdrow">
...
<?
//정렬 기능
if($sod == "desc") $sod_sort = "asc";
else $sod_sort = "desc";
$sort_name = $PHP_SELF."?page=".$page."&sst=pibohum_base.name&sod=".$sod_sort;
$sort_position = $PHP_SELF."?page=".$page."&sst=pibohum_base_opt.position&sod=".$sod_sort;
$sort_jumin_no = $PHP_SELF."?page=".$page."&sst=pibohum_base.jumin_no&sod=".$sod_sort;
$sort_in_day = $PHP_SELF."?page=".$page."&sst=pibohum_base.in_day&sod=".$sod_sort;
$sort_dept = $PHP_SELF."?page=".$page."&sst=pibohum_base_opt.dept_1&sod=".$sod_sort;
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
  ...
  $list = $i%2;
	...
	//부서
	if($row[dept_1]) {
		$sql_dept = " select * from com_code_list where item='dept' and com_code='$code' and code = '$row[dept_1]' ";
...
?>
									...
									<td nowrap class="ltrow1_center_h55">
										...
										<br />
<?
if($gubun == 1) $staff_state = "(휴직)";
else $staff_state = "";
if($out_day != "-") $staff_state = "(퇴사)";
else if(!$staff_state) $staff_state = "";
echo "<span style='color:blue;'>".$staff_state."</span>";
?>
									</td>
									<td nowrap class="ltrow1_center_h55">
										<div id="btn_bsget_82">
