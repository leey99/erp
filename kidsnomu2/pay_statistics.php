...
//사번
if($search_sabun) {
	$sql_search .= " and sabun='$search_sabun' ";
}
	...
	if(cnt==0) {
	 ...
	 return;
		...
		if(confirm("정말 삭제하시겠습니까?")) {
			...
			frm.action="pay_statistics_delete.php";
			frm.submit();
		} else {
			return;
		}
...
?>
											...
											<select name="search_sabun" class="selectfm" onChange="goSearch();">
												<option value="">전체</option>
<?
//사원정보
...
?>
												<option value='<?=$sabun?>' <? if($search_sabun == $sabun) echo "selected"; ?> ><?=$row_sabun['name']?> (<?=$row_sabun['in_day']?>)</option>
<?
}
?>
											</select>
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:fixed;">
								...
								for ($i=0; $row_month=sql_fetch_array($result_month); $i++) {
									...
									if($member['mb_level'] == 6) {
										...
										$url_form2 = "javascript:alert_demo();";
										$url_pay_ledger = "javascript:alert_demo();";
									} else {
										$url_form = "javascript:printPayList('$row_month[year]','$row_month[month]');";
										$url_form2 = "pay_stubs_all_excel.php?code=$code&search_year=$search_year&search_month=$search_month";
								...
								<tr class="list_row_now_wh" onMouseOver="this.className='list_row_over';" onMouseOut="this.className='list_row_now_wh';">
									...
									<td nowrap class="ltrow1_center_h22">
										<table border=0 cellpadding=0 cellspacing=0 style=display:inline;><tr><td width=2></td><td><img src=images/btn_lt.gif></td><td background=images/btn_bg.gif class=ftbutton1 nowrap><a href="<?=$url_form2?>" target="">급여명세서</a></td><td><img src=images/btn_rt.gif></td><td width=2></td></tr></table> 
									</td>
								</tr>
								</tr>
								<?
						...
						<form name="dataForm" method="post">
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:fixed;">
								<tr>
									...
									<td nowrap class="tdhead_center" width="80">기본급</td>
									<td nowrap class="tdhead_center" width="80">과세수당</td>
									<td nowrap class="tdhead_center" width="76">비과세</td>
									<td nowrap class="tdhead_center" width="80">임금계</td>
									<td nowrap class="tdhead_center" width="80">공제계</td>
									<td nowrap class="tdhead_center" width="">지급액</td>
									<td nowrap class="tdhead_center" width="80">급여명세서</td>
								</tr>
<?
//echo $sql;
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
	...
	//과세수당 / b5~8 기타수당(과세)
	$taxation = $row_pay['ext'] + $row_pay['night'] + $row_pay['hday'] + $row_pay['annual_paid_holiday'] + $row_pay['g1'] + $row_pay['g2'] + $row_pay['g3'] + $row_pay['g4'] + $row_pay['g5'] + $row_pay['g6'] + $row_pay['g7'] + $row_pay['g8'] + $row_pay['b5'] + $row_pay['b6'] + $row_pay['b7'] + $row_pay['b8'];
	//비과세 / b1~4 기타수당(비과세)
	$tax_exemption = $row_pay['b1'] + $row_pay['b2'] + $row_pay['b3'] + $row_pay['b4'];
	//서식 링크
	if($member['mb_level'] == 6) {
		$url_form = "javascript:alert_demo();";
	} else {
...
?>
								<tr class="list_row_now_wh" onMouseOver="this.className='list_row_over';" onMouseOut="this.className='list_row_now_wh';">
									<td nowrap class="ltrow1_center_h22"><input type="checkbox" name="idx" value="<?=$row[com_code]?>_<?=$row[sabun]?>_<?=$row[year]?>_<?=$row[month]?>" class="no_borer"></td>
									...
									<td nowrap class="ltrow1_center_h22"><?=number_format($taxation)?></td>
