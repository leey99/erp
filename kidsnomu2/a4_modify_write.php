...
// 수정 상태만 작동
if($w == "u") {
	$sql1 = " select * from a4_modify where id = $id ";
	//echo $sql1;
...
function checkData() {
	...
	frm.action = "a4_modify_update.php";
	frm.submit();
	return;
}
...
//월보수변경 근로자 정보 변경
function modify_name_change(obj_text) {
	var frm = document.dataForm;
	obj_value = obj_text.split("_");
	frm.modify_code.value = obj_value[0];
	frm.modify_name.value = obj_value[1];
	frm.modify_ssnb.value = obj_value[2];
}
//입사자 추가자 정보 변경
function modify_name_change_add(n, obj_text) {
	var frm = document.dataForm;
	obj_value = obj_text.split("_");
	getId('modify_code_'+n).value = obj_value[0];
	getId('modify_name_'+n).value = obj_value[1];
	getId('modify_ssnb_'+n).value = obj_value[2];
}
</script>
<? include "./inc/top.php"; ?>
								...
								<div style="height:10px;font-size:0px;line-height:0px;"></div>
									...
									<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable1" style="table-layout:fixed">
										...
										<tr>
											...
											<td nowrap class="tdrow">
												<input name="modify_name" type="text" class="textfm" style="width:100px;" value="<?=$row1[modify_name]?>" maxlength="25">
												<input name="modify_code" type="hidden" value="<?=$row1['modify_code']?>">
												<select name="modify_sabun" class="selectfm" onchange="modify_name_change(this.value);">
...
for($i=0; $row_sabun=sql_fetch_array($result_sabun); $i++) {
	$sabun_base = $row_sabun['sabun'];
	$sabun_name = $row_sabun['name'];
	$jumin_no = $row_sabun['jumin_no'];
?>
													<option value='<?=$sabun_base?>_<?=$sabun_name?>_<?=$jumin_no?>'><?=$sabun_name?> (<?=$row_sabun['in_day']?>)</option>
<?
	//사원 선택 Select Box 변수 저장
	$sabun_array .= "<option value='".$sabun_base."_".$sabun_name."_".$jumin_no."'>".$sabun_name." (".$row_sabun['in_day'].")</option>";
}
?>
												</select>
											</td>
										...
										<tr>
											...
											<td nowrap class="tdrow">
												<input name="modify_salary" type="text" class="textfm" style="width:150px;ime-mode:inactive;" onkeypress="only_number()" value="<?=$row1[modify_salary]?>" maxlength="25" onkeyup="checkThousand(this.value, this,'Y')">
											</td>
...
?>
										...
										<tr>
											...
											<td nowrap class="tdrow" colspan="3">
												<input name="modify_note" type="text" class="textfm" style="width:100%;" value="<?=$row1[modify_note]?>" maxlength="25">
											</td>
										</tr>
									</table>
...
$modify_cnt_start = $i;
$modify_cnt =20;
for($i=$modify_cnt_start;$i<=$modify_cnt;$i++) {
?>
									modify_name[<?=$i?>] = "";
									modify_ssnb[<?=$i?>] = "";
									modify_date[<?=$i?>] = "";
...
<?
if($sabun) {
	$sql2 = " select * from pibohum_base where com_code='$code' and sabun='$sabun' ";
...
?>
var frm = document.dataForm;
frm.modify_name.value = "<?=$row2['name']?>";
frm.modify_ssnb.value = "<?=$row2['jumin_no']?>";
<? if($row2['apply_gy'] == "0") { ?>frm.misgy.checked = true;<? }?>
<? if($row2['apply_sj'] == "0") { ?>frm.missj.checked = true;<? }?>
<? if($row2['apply_km'] == "0") { ?>frm.miskm.checked = true;<? }?>
<? if($row2['apply_gg'] == "0") { ?>frm.misgg.checked = true;<? }?>
<?
	}
}
//echo $modify_count;
...
?>
									...
									function modify_plus(n){
										...
										if(frm.modify_count.value > 20) {
											...
											tCell.colSpan = 2;
											tCell.innerHTML = "<input name=\"modify_name_[]\" id=\"modify_name_"+n+"\" type=\"text\" class=\"textfm\" style=\"width:100px;\" value=\""+modify_name[n]+"\" maxlength=\"25\"><input name=\"modify_code_[]\" id=\"modify_code_"+n+"\" type=\"hidden\" value=\"\"><select name=\"modify_sabun_[]\" class=\"selectfm\" onchange=\"modify_name_change_add("+n+", this.value);\"><option value=\"______________\">선택</option><?=$sabun_array?></select>";
											tCell = tRow.insertCell();
											tCell.className = "tdrowk"; 
											tCell.colSpan = 2;
											...
											tCell.className = "tdrow"; 
											tCell.colSpan = 2;
											tCell.innerHTML = "<input name=\"modify_ssnb_[]\" id=\"modify_ssnb_"+n+"\" type=\"text\" class=\"textfm\" style=\"width:100px;\" value=\""+modify_ssnb[n]+"\" maxlength=\"14\">";
											...
											tCell.className = "tdrow"; 
											tCell.colSpan = 2;
											tCell.innerHTML = "<input name=\"modify_salary_[]\" type=\"text\" class=\"textfm\" style=\"width:150px;ime-mode:inactive;\" onkeypress=\"only_number()\" value=\""+modify_salary[n]+"\" maxlength=\"25\" onkeyup=\"checkThousand(this.value, this,'Y')\">";
											tCell = tRow.insertCell();
											tCell.className = "tdrowk"; 
											tCell.colSpan = 2;
											...
											tCell.colSpan = 6;
											tCell.innerHTML = "<input name=\"modify_note_[]\" type=\"text\" class=\"textfm\" style=\"width:100%;\" value=\""+modify_note[n]+"\" maxlength=\"25\">";
									...
									<div style="margin:8px 0 0 0;">
										※ 연금보험료 보수변경 : 연금보험료의 월보수변경은 변경전 보수월액 대비 ±20% 이상 차이날 경우에만 변경신청이 가능합니다. 변경신청 시 참고바랍니다.
									</div>
									<div style="height:10px;font-size:0px;line-height:0px;"></div>
