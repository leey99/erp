...
function retirement_age_rule_change() {
	...
	var retirement_age_rule1 = f.retirement_age_rule1.value;
	var retirement_age_rule2 = f.retirement_age_rule2.value;
	f.retirement_age_rule.value = "만 "+retirement_age_rule1+"세가 되는 날이 속하는 달의 "+retirement_age_rule2+"일";
...
?>
										...
										<input type="checkbox" name="pay_day_now_month" value="1" <?=$pay_day_now_month_checked?>> 당월
...
?>
									...
									<td nowrap class="tdrowk"><img src="./images/icon_02.gif" width="2" height="2" style="margin:0 5 3 0">임금계산기간</td>
									<td nowrap  class="tdrow">
<?
...
?>
								...
								<tr>
									<td nowrap class="tdrowk"><img src="./images/icon_02.gif" width="2" height="2" style="margin:0 5 3 0">원천세신고</td>
									<td nowrap  class="tdrow">
<?
if($row['account_report_way'] == "1") $sel_account_report_way1 = "selected";
else if($row['account_report_way'] == "2") $sel_account_report_way2 = "selected";
?>
										<select name="account_report_way" class="selectfm">
											<option value="">선택</option>
											<option value="1" <?=$sel_account_report_way1?>>반기</option>
											<option value="2" <?=$sel_account_report_way2?>>월별</option>
										</select>
									</td>
									<td nowrap class="tdrowk"><img src="./images/icon_02.gif" width="2" height="2" style="margin:0 5 3 0">정년연령</td>
									<td nowrap  class="tdrow">
										<input name="retirement_age" type="text" class="textfm" style="width:40px;ime-mode:disabled;" value="<?=$row1[retirement_age]?>" maxlength="100"> 예) 50
									</td>
...
?>
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable1" style="table-layput:fixed">
								<tr>
									<td nowrap class="tdrowk"><img src="./images/icon_02.gif" width="2" height="2" style="margin:0 5 3 0">관리관리점</td>
									<td nowrap  class="tdrow">
<?
if($member['mb_level'] >= 5) {
...
?>
									...
									<td nowrap class="tdrowk"><img src="./images/icon_02.gif" width="2" height="2" style="margin:0 5 3 0">버전</td>
									<td nowrap  class="tdrow">
<?
if($row['lawwiz_code'] == "1") {
	$sel_lawwiz_code1 = "selected";
	$lawwiz_code = "1";
} else if($row['lawwiz_code'] == "2") {
	$sel_lawwiz_code2 = "selected";
	$lawwiz_code = "2";
} else if($row['lawwiz_code'] == "f") {
	$sel_lawwiz_code3 = "selected";
	$lawwiz_code = "f";
}
if($member['mb_level'] >= 7) {
?>
										<select name="lawwiz_code" class="selectfm">
											...
											<option value="f" <?=$sel_lawwiz_code3?>>Lite</option>
										</select>
<?
} else {
	echo $lawwiz_code;
...
if(!$persons) {
	...
	if(!$hday4) $hday4 = "삼일절";
	if(!$hday5) $hday5 = "어린이날";
	if(!$hday6) $hday6 = "석가탄신일";
	if(!$hday7) $hday7 = "현충일";
	if(!$hday8) $hday8 = "광복절";
	if(!$hday9) $hday9 = "추석";
	if(!$hday10) $hday10 = "개천절";
	if(!$hday11) $hday11 = "한글날";
	if(!$hday12) $hday12 = "성탄절";

	if(!$retirement_age_rule) $retirement_age_rule = "만 50세가 되는 날";
