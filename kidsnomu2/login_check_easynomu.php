...
//키즈노무 버전별 로그인 후 메인 페이지
$sql_com = " select * from com_list_gy where t_insureno = '$mb[mb_id]' ";
$row_com = sql_fetch($sql_com);
$lawwiz_code = $row_com['lawwiz_code'];
if($lawwiz_code == "2") {
	$url = "%2Fkidsnomu2%2Fmain.php";
} else if($lawwiz_code == "f") {
	$url = "%2Fkidsnomu_lite%2Fmain.php";
} else {
	$url = "%2Fkidsnomu%2Fmain.php";
}
if ($url) 
{
    $link = urldecode($url);
