	...
	if($excel) {
		...
		if(file_exists($upfile_path)) {
			...
			for ($i = 0; $i < $maxRow; $i++) {
				...
				$excel_b1[$excel_sabun] = trim($objWorksheet->getCell('O' . $k)->getValue());
...
function pay_json() {
	...
	for(i=1;i<=total;i++) {
		...
		jData += '"money_year_'+i+'":"'+money_year+'","b1_'+i+'":"'+b1+'","workhour_total_'+i+'":"'+workhour_total+'","money_total_'+i+'":"'+money_total+'","money_for_tax_'+i+'":"'+money_for_tax+'",';
	...
	$.ajax({
		type: 'post',
		dataType: 'json',
		url: 'pay_excel_beistand_json.php',
...
function excel_upload(obj) {
	...
	frm.action = "pay_excel_beistand_upload.php";
	frm.submit();
	return;
}
//]]>
...
if($member['mb_profile'] == "guest") {
	...
	$url_excel_save = "pay_excel_beistand_save.php?code=".$com_code."&data=".$data."&select_type=".$select_type."&search_year=".$search_year."&search_month=".$search_month;
														...
														<td>  
															<table width="<?=$pay_list_width?>" height="79" border="0" cellpadding="0" cellspacing="1" class="bgtable">
																...
																<tr>
																	...
																	<td class="tdhead_center" rowspan="2" width="70">교통<br>지원금</td>
																	<td class="tdhead_center" rowspan="2" width="70">가산<br />수당</td>
...
?>
														...
														<td nowrap class="ltrow1_center_h24" width="70"><span id="money_year_<?=$k?>"><?=$annual_paid_holiday?></span></td><!--교통지원금-->
														<td nowrap class="ltrow1_center_h24" width="70">
															<span id="workhour_total_<?=$k?>" style="display:none;"><?=$workhour_total?></span><!--임금산출 총시간-->
															<span id="b1_<?=$k?>"><?=$b1?></span><!--가산수당-->
														</td>
...
function cal_pay(idx) {
	...
	var workhour_total,money_year,b1;
	...
	b1      = toInt(getId('b1_'+idx).innerHTML);	//가산수당
	...
	money_total = parseInt( (workhour_1*<?=$money_time1_com?>) + (workhour_2*<?=$money_time2_com?>) + (workhour_3*<?=$money_time3_com?>) + (workhour_1_hday*<?=$money_time1_hday_com?>) + (workhour_2_hday*<?=$money_time2_hday_com?>) + (workhour_3_hday*<?=$money_time3_hday_com?>) + money_year + b1 );

	money_pay = parseInt( (workhour_1*<?=$money_time1?>) + (workhour_2*<?=$money_time2?>) + (workhour_3*<?=$money_time3?>) + (workhour_1_hday*<?=$money_time1_hday?>) + (workhour_2_hday*<?=$money_time2_hday?>) + (workhour_3_hday*<?=$money_time3_hday?>) + (workhour_edu*<?=$money_time_edu?>) + (workhour_phone*<?=$money_time_phone?>) + money_year + b1 );
...
<?
if($search_year==2016 && $search_month=="05") {
?>
	//강대진 활동보조인 2016년 5월 강제 퇴직연금 지급 160610
	if(f['sabun_'+idx].value == "0282") {
		retirement_pension = Math.ceil((money_for_tax / 12)/10)*10;
	}
<?
}
