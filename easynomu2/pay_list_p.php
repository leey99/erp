...
function goInput(mode){
	...
	*/
	...
	for(i=1;i<=<?=$total_count?>;i++) {
		...
		f["money_period_var_"+k].value = f.money_period[i].value;
	...
	f.mode.value = mode;
	f.action = "pay_update_type_p.php";
	f.submit();
}
//급여반영 입력폼
...
function cal_pay2(idx){
	...
	var week_hday,year_hday,money_base,money_ext,money_night,money_hday,money_ext_add,money_night_add,money_hday_add,money_week,money_year,money_period;
	...
	money_period    = toInt(f.money_period  [idx].value);	// 생리수당
...
function focusInClass(idx) {
	...
	frm.money_period[idx].className = "textfm_trans";
...
function focusOutClass(idx) {
	...
	if(frm.idx[idx].checked == false) {
		...
		frm.money_period[idx].className = "textfm";
...
<?
include "./inc/pay_list_include_p.php";
?>
								<!--리스트 -->
...
//부서별 검색 160129
if($stx_dept) $sql_search_manual = " and dept_code = '$stx_dept' ";
//수동입력
$sql_manual = " select * from pibohum_base_pay where com_code='$com_code' and year = '$search_year' and month = '$search_month' $sql_search_manual and w_date != '0000-00-00' order by manual desc ";
//echo $sql_manual;
$row_manual = sql_fetch($sql_manual);
//echo "//수동입력 ".$row_manual[manual];
														...
														<td>  
															<table width="<?=$pay_list_width?>" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:;">
																<tr>
																	...
																	<td class="tdhead_center" colspan="8">기본급 및 제수당 </td>
																	<td class="tdhead_center" rowspan="4" width="18"></td>
																...
																<tr>
																	...
																	<td class="tdhead_center" rowspan="2" width="74">통상시급</td>
																	<td class="tdhead_center" rowspan="2" width="74">기본시급</td>
																	...
																	<td class="tdhead_center" colspan="5">법정수당(과세)</td>
...
?>
																...
																<tr>
																	<td class="tdhead_center" width="88"><input type="checkbox" name="manual_ext" <?=$check_manual_ext?> onclick="check_manual(this);" value="1" title="수동입력" style="vertical-align:middle;"> 연장근로</td>
																	<td class="tdhead_center" width="88"><input type="checkbox" name="manual_night" <?=$check_manual_night?> onclick="check_manual(this);" value="1" title="수동입력" style="vertical-align:middle;"> 야간근로</td>
																	<td class="tdhead_center" width="88"><input type="checkbox" name="manual_hday" <?=$check_manual_hday?> onclick="check_manual(this);" value="1" title="수동입력" style="vertical-align:middle;"> 휴일근로</td>
																	<td class="tdhead_center" width="87">연차수당</td>
																	<td class="tdhead_center" width="87">생리수당</td>
...
?>
											<div id="spanLeft" style="width:200px;height:<?=$spanMain_height?>px;overflow:hidden">
												<div style="display:none;">
													...
													<input type="hidden" name="money_period">
...
//직위 급여DB에 존재시 급여DB 우선 표시 160106
$sql_pay = " select * from pibohum_base_pay where com_code='$row[com_code]' and sabun='$row[sabun]' and year = '$search_year' and month = '$search_month' and w_date != '0000-00-00' order by w_date desc, w_time desc limit 0, 1 ";
$result_pay = sql_query($sql_pay);
$row_pay = mysql_fetch_array($result_pay);
if($row_pay['position_txt']) {
	$row2['position'] = $row_pay['position'];
	$row_position['name'] = $row_pay['position_txt'];
}
?>
											...
											<div id="spanMain" style="width:100%;height:<?=$spanMain_height?>px;overflow-x:hidden;overflow-y:auto;" onScroll="spanLeft.scrollTop = this.scrollTop; spanTop.scrollLeft = this.scrollLeft;">
												<table width="<?=$pay_list_width?>" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:fixed;">
													...
													for ($i=0; $row=sql_fetch_array($result); $i++) {
														...
														//직위 급여DB 우선 표시
														if($row4['position_txt']) {
															$row2['position'] = $row4['position'];
															$row_position['name'] = $row4['position_txt'];
														}
													?>
...
if($row4['money_month'] == 0 || $data == "load") {
	...
	$money_period = $row4[money_period];
...
?>
														...
														<input type="hidden" name="money_period_var_<?=$i?>">
														...
														<td nowrap class="ltrow1_center_h24" width="73"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="money_hour_ts" value="<?=number_format($money_hour_ts)?>"></td><!-- 통상임금(시간급) -->
														<td nowrap class="ltrow1_center_h24" width="74"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="money_time" value="<?=number_format($money_time)?>"></td><!-- 기본시급 -->
<?
//기본급 수동입력 클래스 설정
...
?>
														...
														<td nowrap class="ltrow1_center_h24" width="88"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="<? if($check_manual_ext != "checked") echo "textfm5"; else echo "textfm"; ?>" name="money_ext" value="<?=number_format($money_ext)?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay('<?=$k?>');"></td><!-- 연장근로수당 -->
														<td nowrap class="ltrow1_center_h24" width="88"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="<? if($check_manual_night != "checked") echo "textfm5"; else echo "textfm"; ?>" name="money_night" value="<?=number_format($money_night)?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay('<?=$k?>');"></td><!-- 야간근로수당 -->
														<td nowrap class="ltrow1_center_h24" width="88"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="<? if($check_manual_hday != "checked") echo "textfm5"; else echo "textfm"; ?>" name="money_hday" value="<?=number_format($money_hday)?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay('<?=$k?>');"></td><!-- 휴일근로수당 -->
														<td nowrap class="ltrow1_center_h24" width="86"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm"  name="money_year" value="<?=number_format($annual_paid_holiday)?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay('<?=$k?>');"></td><!-- 연차수당 -->
														<td nowrap class="ltrow1_center_h24" width="87"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm"  name="money_period" value="<?=number_format($money_period)?>" onKeyUp="checkThousand(this.value, this,'Y');cal_pay('<?=$k?>');"></td><!-- 연차수당 -->
														...
														<td nowrap class="ltrow1_center_h24" width="60"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="commify(this.value, this);javascript:focusOutClass('<?=$k?>');" class="textfm" name="minus" value="<?=number_format($row4[minus])?>" onKeyUp="checkThousand_negative(this.value, this,'Y');cal_pay2('<?=$k?>');"></td><!-- 기타공제 -->
...
function cal_pay(idx) {
	...
	var week_hday,year_hday,money_base,money_time,money_ext,money_night,money_hday,money_ext_add,money_night_add,money_hday_add,money_week,money_year,money_period;
	var money_g1,money_g2,money_g3,money_g4,money_g5,money_e1,money_e2,money_e3,money_e4,money_e5,money_e6,money_e7,money_e8;
...
?>
		...
		if( workhour_total != 0 ){
			...
			if(f.check_money_min_yn[idx].value == "Y") {
				...
				if(money_month_old == money_month) {
					...
					if(!f.money_month_fix.checked) money_base = money_month - (money_ext + money_night + money_hday) - (money_g_sum + money_e_sum) - (money_year + money_period);
	...
	//가불 임금계(입금총액)에 포함
	money_total = money_base + (money_ext + money_hday + money_night + money_year + money_period) + (money_ext_add + money_hday_add + money_night_add) + (money_g_sum + money_e_sum) - etc2;
<?
//포밍 가불 임금계 마이너스 처리 160901
if($com_code == "20602") {
	echo "money_total -= etc;";
}
?>
	//money_for_tax 과세소득 
...
?>
	...
	money_period = number_format(money_period);
