...
if($chk_data) {
	for( $i=0 ; $i < $check_cnt ; $i++) {
		...
		$sql = "select * from pibohum_base_pay_h where $sql_where ";
		//echo count($idx);
		//echo $sql;
		//exit;
		$row = sql_fetch($sql);
		if (!$row[sabun]) {
			...
			sql_query("delete from pibohum_base_pay_h where $sql_where ");
		}
	}
}
goto_url("./pay_ledger_list_beistand.php?search_year=$search_year");
?>
