...
$sql_order = " order by $sst $sod $sst2 $sod2 $sst3 $sst4 $sst5 $sst6 ";

$sql = " select count(distinct year, month, sabun) as cnt
         $sql_common
...
$colspan = 14;
$qstr = "search_year=".$search_year."&search_month=".$search_month."&employee=".$employee;
?>
<head>
...
?>
										...
										<td nowrap class="tdrowk"><img src="/img/ims/icon_02.gif" width="2" height="2" style="margin:0 5 3 0">근로자</td>
										<td nowrap class="tdrow">
											<!-- 근로자리스트사항 -->
											<select name='employee' id='employee' class='select' onchange=''>
...
for ($i=0; $row2=sql_fetch_array($result2); $i++) {
	// 사업장명 : 사업장코드
	$sql_com = " select com_name from $g4[com_list_gy] where com_code = '$row2[com_code]' ";
	$row_com = sql_fetch($sql_com);
	$com_name = $row_com['com_name'];
	$com_name = cut_str($com_name, 21, "..");
	$name = cut_str($row2['name'], 6, "..");
	$idx = $row2['com_code']."_".$row2['sabun'];
	if($employee) {
		$code_id_array = explode("_", $employee);
		$id = $code_id_array[1];
	}
?>
												<option value="<?=$idx?>" <? if($id == $row2['sabun']) echo "selected"; ?> ><?=$name?> (<?=$row2['in_day']?>)</option>
<?
}
if ($i == 0)
...
?>
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:fixed;">
								<tr>
									...
									<td nowrap class="tdhead_center" width="90">등록일시</td>
									...
									<td nowrap class="tdhead_center" width="76">급여대장</td>
								</tr>
								<?
								// 리스트 출력
								$sst2 = "year desc";
								...
								$rows2 = 15;
								$total_page2  = ceil($total_count_month / $rows2);  // 전체 페이지 계산

								if (!$page2) $page2 = 1; // 페이지가 없으면 첫 페이지 (1 페이지)
								$from_record2 = ($page2 - 1) * $rows2; // 시작 열을 구함
								...
								for ($i=0; $row_month=sql_fetch_array($result_month); $i++) {
									...
									if($year != $old_year || $month != $old_month || $i == 0) {
										$no = $total_count_month - $i - ($row_months*($page2-1));
										$list = $i%2;
										//사업장 코드 / 사번 / 코드_사번
										...
										if($member['mb_profile'] == "guest") {
											...
											$url_form = "javascript:printPayList('$row_month[year]','$row_month[month]');";
								...
								<tr class="list_row_now_wh" onMouseOver="this.className='list_row_over';" onMouseOut="this.className='list_row_now_wh';">
									...
									<td nowrap class="ltrow1_center_h22"><?=$reg_day?><br><?=$reg_time?></td>
...
?>
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:fixed;">
								<tr>
									...
									<td nowrap class="tdhead_center" width="52">연도</td>
									<td nowrap class="tdhead_center" width="38">월</td>
									<td nowrap class="tdhead_center" width="38">사번</td>
									...
									<td nowrap class="tdhead_center" width="80">기본월급</td>
									<td nowrap class="tdhead_center" width="86">과세소득</td>
								...
								$sql = " select *
													$sql_common
													$sql_search $sql_search2 and w_date='$reg_day' and w_time='$reg_time'
													order by sabun asc
													limit $from_record, $rows ";
								//echo $sql;
								...
								for ($i=0; $row=sql_fetch_array($result); $i++) {
									...
									$sum_money_for_tax += $row_pay['money_for_tax'];
									$sum_money_total += $row_pay['money_total'];
								?>
								<tr class="list_row_now_wh" onMouseOver="this.className='list_row_over';" onMouseOut="this.className='list_row_now_wh';">
									...
									<td nowrap class="ltrow1_center_h22"><?=number_format($row['money_hour_ds'])?></td>
								...
								<tr class="list_row_now_wh" onMouseOver="" onMouseOut="">
									...
									<td nowrap class="ltrow1_center_h22" colspan="7">합계</td>
									<td nowrap class="ltrow1_center_h22"></td>
									<td nowrap class="ltrow1_center_h22"></td>
									<td nowrap class="ltrow1_center_h22"></td>
									<td nowrap class="ltrow1_center_h22"><?=number_format($sum_money_for_tax)?></td>
									<td nowrap class="ltrow1_center_h22"><?=number_format($sum_money_total)?></td>
									<td nowrap class="ltrow1_center_h22"></td>
								</tr>
								<?
									}
							...
							<table width="60%" align="center" border="0" cellpadding="0" cellspacing="0">
								<tr>
									<td height="40">
										<div align="center">
<?
//echo $config[cf_write_pages].", ".$page.", ".$total_page;
