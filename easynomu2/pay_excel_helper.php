...
//급여관리 DB (급여반영) 년월
$sql_w_date = " select * from pibohum_base_pay_helper where com_code='$com_code' and year = '$search_year' and month = '$search_month' ";
...
function cal_pay2(idx){
	var f = document.dataForm;
	var workhour_1, workhour_2;
	var workhour_total;
	var money_ext,money_night,money_hday,money_year;
	var money_for_tax,money_yun,money_health,money_yoyang,money_goyong,tax_so,tax_jumin,etc,etc2,money_gongje,money_result;
	var c_money_yun,c_money_health,c_money_yoyang,c_money_goyong,c_money_sanjae,c_money_gongje;

	workhour_1    = toFloat(getId('workhour_1_'+idx).innerHTML);	//동부권 평근
...
if($data == "excel") {
	//엑셀 업로드 파일 추출 151214
	$sql_opt2 = " select pay_excel_2 from com_list_gy_opt2 where com_code='$com_code' ";
	$result_opt2 = sql_query($sql_opt2);
	$row_opt2 = mysql_fetch_array($result_opt2);
	if($row_opt2['pay_excel_2']) $excel = $row_opt2['pay_excel_2'];
	//엑셀 리더
	...
	if($excel) {
		...
		if(file_exists($upfile_path)) {
			...
			for ($i = 0; $i < $maxRow; $i++) {
				...
				$excel_workhour_1[$excel_sabun] = trim($objWorksheet->getCell('E' . $k)->getValue()); 
				$excel_workhour_2[$excel_sabun] = trim($objWorksheet->getCell('F' . $k)->getValue());
				$excel_annual_paid_holiday[$excel_sabun] = trim($objWorksheet->getCell('G' . $k)->getValue());

				$excel_yun[$excel_sabun] = trim($objWorksheet->getCell('H' . $k)->getValue());
				$excel_health[$excel_sabun] = trim($objWorksheet->getCell('I' . $k)->getValue());
				$excel_yoyang[$excel_sabun] = trim($objWorksheet->getCell('J' . $k)->getValue());
				$excel_goyong[$excel_sabun] = trim($objWorksheet->getCell('K' . $k)->getValue());
				$excel_tax_so[$excel_sabun] = trim($objWorksheet->getCell('L' . $k)->getValue());
				$excel_tax_jumin[$excel_sabun] = trim($objWorksheet->getCell('M' . $k)->getValue());
				$excel_minus[$excel_sabun] = trim($objWorksheet->getCell('N' . $k)->getValue());

				$excel_c_yun[$excel_sabun] = trim($objWorksheet->getCell('O' . $k)->getValue());
				$excel_c_health[$excel_sabun] = trim($objWorksheet->getCell('P' . $k)->getValue());
				$excel_c_yoyang[$excel_sabun] = trim($objWorksheet->getCell('Q' . $k)->getValue());
				$excel_c_goyong[$excel_sabun] = trim($objWorksheet->getCell('R' . $k)->getValue());
				$excel_c_sanjae[$excel_sabun] = trim($objWorksheet->getCell('S' . $k)->getValue());
...
function pay_json() {
	...
	for(i=1;i<=total;i++) {
		...
		jData += '"workhour_1_'+i+'":"'+workhour_1+'","workhour_2_'+i+'":"'+workhour_2+'",';
		jData += '"money_year_'+i+'":"'+money_year+'","workhour_total_'+i+'":"'+workhour_total+'","money_for_tax_'+i+'":"'+money_for_tax+'",';

		jData += '"money_yun_'+i+'":"'+money_yun+'","money_health_'+i+'":"'+money_health+'","money_yoyang_'+i+'":"'+money_yoyang+'","money_goyong_'+i+'":"'+money_goyong+'",';
		jData += '"tax_so_'+i+'":"'+tax_so+'","tax_jumin_'+i+'":"'+tax_jumin+'",';
...
function excel_upload(obj) {
	...
	frm.action = "pay_excel_helper_upload.php";
	frm.submit();
	return;
}
//]]>
...
if($member['mb_profile'] == "guest") {
	...
	$url_excel_save = "pay_excel_helper_save.php?code=".$com_code."&data=".$data."&select_type=".$select_type."&search_year=".$search_year."&search_month=".$search_month;
	$url_excel_upload = "javascript:excel_upload('pay_excel_2');";
}
?>
									<a href="<?=$url_excel_save?>"><img src="images/btn_excel_save.png" style="vertical-align:middle;" border="0" alt="엑셀저장" /></a>
									<input name="pay_excel_2" type="file" class="textfm_search" style="width:320px;margin:0 0 0 10px;vertical-align:middle;" />
								...
								<table border=0 cellspacing=0 cellpadding=0> 
									<tr> 
										...
										<td width="" align="left">
											<div id="step2" style="cursor:pointer;margin:0 5px 0 0"><img src="images/pay_step2_h.gif" id="pay_step2" style="display:none;" /></div>
										</td> 
										<td width="" align="left">
...
//수동입력
$sql_manual = " select * from pibohum_base_pay_helper where com_code='$com_code' and year = '$search_year' and month = '$search_month' and w_date != '0000-00-00' order by manual desc ";
//echo $sql_manual;
													...
													<tr>
														<td>  
															<table width="<?=$pay_list_width?>" height="79" border="0" cellpadding="0" cellspacing="1" class="bgtable">
																<tr>
																	<td class="tdhead_center" colspan="4">근로시간(월) </td>
																	<td class="tdhead_center" colspan="1">임금합계</td>
																	<td class="tdhead_center" colspan="7">공제(근로자)</td>
																	<td class="tdhead_center" rowspan="3" width="80">공제후<br>지급액 </td>
																...
																<tr>
																	<td class="tdhead_center" colspan="1">동부권</td>
																	<td class="tdhead_center" colspan="1">남서부권</td>
																	<td class="tdhead_center" rowspan="2" width="69">교통<br>지원금</td>
																	<td class="tdhead_center" rowspan="2" width="42">총<br>근로<br />시간</td>
																	...
																	<td class="tdhead_center" rowspan="2" width="70">공제계</td>
																...
																<tr>
																	<td class="tdhead_center" rowspan="1" width="45">평근<br /><?=number_format($money_time1_helper)?></td>
																	<td class="tdhead_center" rowspan="1" width="52">평근<br /><?=number_format($money_time2_helper)?></td>

																	<td class="tdhead_center" rowspan="1" width="55">국민연금<br /><input type="checkbox" name="manual_yun" <?=$check_manual_yun?> onclick="check_manual_yun_func(this)" value="1" title="수동입력" style="vertical-align:middle;"></td>
																	<td class="tdhead_center" rowspan="1" width="55">건강보험</td>
																	<td class="tdhead_center" rowspan="1" width="55">장기요양</td>
																	<td class="tdhead_center" rowspan="1" width="55">고용보험</td>
																	<td class="tdhead_center" rowspan="1" width="50">소득세</td>
																	<td class="tdhead_center" rowspan="1" width="50">주민세</td>
...
?>
														<td nowrap class="ltrow1_center_h24" width="45">
<?
//echo $total_count;
if($k < $total_count) {
	$k_next = $k+1;
...
?>
															<span id="workhour_1_<?=$k?>"><?=$workhour_1?></span><!--동부권 평근-->
														</td>
														<td nowrap class="ltrow1_center_h24" width="52">
															<span id="workhour_2_<?=$k?>"><?=$workhour_2?></span><!--서남부권 평근-->
														</td>
														<td nowrap class="ltrow1_center_h24" width="69"><span id="money_year_<?=$k?>"><?=$annual_paid_holiday?></span></td><!--교통지원금-->
														<td nowrap class="ltrow1_center_h24" width="42"><span id="workhour_total_<?=$k?>"><?=$workhour_total?></span></td><!--임금산출 총시간-->
														...
														<td nowrap class="ltrow1_center_h24" width="55"><span id="money_yun_<?=$k?>"><? if($row4['yun']) echo number_format($row4['yun']);?></span></td><!--국민연금-->
														<td nowrap class="ltrow1_center_h24" width="55"><span id="money_health_<?=$k?>"><? if($row4['health']) echo number_format($row4['health']);?></span></td><!--건강보험-->
														<td nowrap class="ltrow1_center_h24" width="55"><span id="money_yoyang_<?=$k?>"><? if($row4['yoyang']) echo number_format($row4['yoyang']);?></span></td><!--장기요양보험-->
														<td nowrap class="ltrow1_center_h24" width="55"><span id="money_goyong_<?=$k?>"><? if($row4['goyong']) echo number_format($row4['goyong']);?></span></td><!--고용보험-->
														<td nowrap class="ltrow1_center_h24" width="50"><span id="tax_so_<?=$k?>"><? if($row4['tax_so']) echo number_format($row4['tax_so']);?></span></td><!--소득세-->
														<td nowrap class="ltrow1_center_h24" width="50"><span id="tax_jumin_<?=$k?>"><? if($row4['tax_jumin']) echo number_format($row4['tax_jumin']);?></span></td><!--주민세-->

														<td nowrap class="ltrow1_center_h24" width="70"><span id="money_gongje_<?=$k?>"><? if($row4['money_gongje']) echo number_format($row4['money_gongje'])?></span></td><!--공제계-->
														<td nowrap class="ltrow1_center_h24" width="80"><span id="money_result_<?=$k?>"><? if($row4['money_result']) echo number_format($row4['money_result']);?></span></td><!--공제후지급액-->
...
function cal_pay(idx) {
	...
	var workhour_1, workhour_2, money_pay, retirement_pension;
	...
	workhour_total = parseInt( ( workhour_1 + workhour_2 ) * 1000 ) / 1000;
	workhour_total = Math.round(workhour_total*100)/100;

	money_pay = parseInt( (workhour_1*<?=$money_time1_helper?>) + (workhour_2*<?=$money_time2_helper?>) + money_year );
