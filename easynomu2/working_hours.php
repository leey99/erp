...
function rp_sum_func(year, code, id) {
	var ret = window.open("./working_hours_popup.php?year="+year+"&code="+code+"&id="+id, "retirement_popup", "width=590, height=370, toolbar=no, menubar=no, scrollbars=yes, resizable=no" );
	return;
}
</script>
<? include "./inc/top.php"; ?>
...
?>
									...
									<td nowrap class="tdhead_center" width="80" colspan="6">근로시간/급여</td>
								</tr>
								<tr>
									<td nowrap class="tdhead_center" width="74"><?=$now_year-3?></td>
									<td nowrap class="tdhead_center" width="74"><?=$now_year-2?></td>
									<td nowrap class="tdhead_center" width="74"><?=$now_year-1?></td>
									<td nowrap class="tdhead_center" width="74"><?=$now_year?></td>
									<td nowrap class="tdhead_center" width="74"><?=$now_year+1?></td>
									<td nowrap class="tdhead_center" width="90">합계</td>
								</tr>
<?
// 리스트 출력
for ($i=0; $row=sql_fetch_array($result); $i++) {
  ...
  $list = $i%2;
	...
	for($y=($now_year-3);$y<=($now_year+1);$y++) {
		...
		$sql_rp_month = " select workhour_total, month from pibohum_base_pay_h where com_code='$code' and sabun='$id' and year='$y' ";
...
?>
									...
									<td nowrap class="ltrow1_center_h55" style="cursor:pointer;" onclick="rp_sum_func('<?=$now_year-3?>','<?=$code?>','<?=$id?>');return false;" onkeypess="this.onclick;"><?=number_format($rp_sum[$now_year-3])?><br /><?=number_format($pay_sum[$now_year-3])?></td>
									<td nowrap class="ltrow1_center_h55" style="cursor:pointer;" onclick="rp_sum_func('<?=$now_year-2?>','<?=$code?>','<?=$id?>');return false;" onkeypess="this.onclick;"><?=number_format($rp_sum[$now_year-2])?><br /><?=number_format($pay_sum[$now_year-2])?></td>
									<td nowrap class="ltrow1_center_h55" style="cursor:pointer;" onclick="rp_sum_func('<?=$now_year-1?>','<?=$code?>','<?=$id?>');return false;" onkeypess="this.onclick;"><?=number_format($rp_sum[$now_year-1])?><br /><?=number_format($pay_sum[$now_year-1])?></td>
									<td nowrap class="ltrow1_center_h55" style="cursor:pointer;" onclick="rp_sum_func('<?=$now_year?>'  ,'<?=$code?>','<?=$id?>');return false;" onkeypess="this.onclick;"><?=number_format($rp_sum[$now_year])?><br /><?=number_format($pay_sum[$now_year])?></td>
									<td nowrap class="ltrow1_center_h55" style="cursor:pointer;" onclick="rp_sum_func('<?=$now_year+1?>','<?=$code?>','<?=$id?>');return false;" onkeypess="this.onclick;"><?=number_format($rp_sum[$now_year+1])?><br /><?=number_format($pay_sum[$now_year+1])?></td>
									<td nowrap class="ltrow1_center_h55"><strong><?=number_format($rp_sum_sum)?><br /><?=number_format($pay_sum_sum)?></strong></td>
								</tr>
<?
}
if ($i == 0)
...
if($member['mb_profile'] == "guest") {
	...
	$url_excel = "working_hours_excel.php?code=".$code."&amp;kind=".$kind;
}
?>
