...
//최저시급 2016년
$money_min_time_year = 2016;
$money_min_time = 6030;
$money_min_month = 6030*209;
...
$sub_title = "급여정보";
$g4[title] = $sub_title." : 사원관리 : ".$member['mb_nick'];
...
?>
									<td nowrap class="tdhead_center" width="72">결정임금</td>
									<td nowrap class="tdhead_center" width="72">기본급</td>
									<td nowrap class="tdhead_center" width="65">제수당</td>
									<td nowrap class="tdhead_center" width="64">기본시급<br>통상시급</td>
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
  ...
  $list = $i%2;
	...
	//월급제만 최저월급 미달 표시 161028
	if($row2['pay_gbn'] != "1") {
		if($money_hour_ms < $money_min_month) {
			$money_hour_ms_color = "style='color:red'";
			$money_hour_ms_text = "최저월급 미달(".$money_min_time_year."년 ".number_format($money_min_month)."원)";
		} else {
			$money_hour_ms_color = "";
			$money_hour_ms_text = "";
		}
	...
	if($money_min_base < $money_min_time) {
		$money_min_base_color = "style='color:red'";
		$money_min_base_text = "최저시급 미달(".$money_min_time_year."년 ".number_format($money_min_time)."원)";
	} else {
		$money_min_base_color = "";
		$money_min_base_text = "";
	}
	//기준시급
...
?>
									...
									<td nowrap class="ltrow1_center_h55"><?=$work_gbn_text?><br><?=$work_gbn?></td>
<?
if($is_admin != "super") {
...
?>
									<td nowrap class="ltrow1_center_h55"><?=$pay_gbn?><br><?=$input_type?></td>
<?
}
?>
									<td nowrap class="ltrow1_center_h55">
...
/*
if($row2['pay_gbn'] != "1") {
	echo number_format($money_month_base);
} else {
	echo number_format($money_hour_ds);
}
*/
echo number_format($money_month_base);
?>
									</td>
									<td nowrap class="ltrow1_center_h55" <?=$money_hour_ms_color?> title="<?=$money_hour_ms_text?>">
<?
/*
	if($row2[pay_gbn] == "0") $pay_gbn = "월급제";
	else if($row2[pay_gbn] == "1") $pay_gbn = "시급제";
...
*/
if($row2['pay_gbn'] != "1") {
	echo number_format($money_hour_ms);
} else {
	echo "-";
}
?>
									</td>
									<td nowrap class="ltrow1_center_h55"><?=number_format($money_allowance)?></td>
									<td nowrap class="ltrow1_center_h55" <?=$money_min_base_color?> title="<?=$money_min_base_text?>"><?=number_format($money_min_base)?><br><?=number_format($money_hour_ts)?></td>
									<td nowrap class="ltrow1_center_h55"><table border=0 cellpadding=0 cellspacing=0 style="display:inline;"><tr><td width=2></td><td><img src=images/btn_lt.gif></td><td background=images/btn_bg.gif class="ftbutton1" nowrap><a href="<?=$url_pay_copy?>" target="">급여복사</a></td><td><img src="images/btn_rt.gif"></td><td width=2></td></tr></table></td>
								</tr>
<?
}
if ($i == 0)
