								...
								for ($i=0; $row=sql_fetch_array($result); $i++) {
									...
									if($search_month || $year != $old_year || $month != $old_month || $i == 0) {
										...
										if($member['mb_profile'] == "guest") {
											...
											$url_pay_ledger = "pay_list.php?w=u&code=".$code."&search_year=".$row[year]."&search_month=".$row[month]."&w_date=".$reg_day."&w_time=".$reg_time;
										...
										if($reg_cnt > 1) {
											$url_pay_ledger_list = $_SERVER['PHP_SELF']."?search_year=".$row[year]."&search_month=".$row[month];
										} else {
