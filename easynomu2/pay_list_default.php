...
if(!$w_date_ok) {
	...
	//연장, 야간, 휴일 수동입력 체크(사원급여정보) 160824
	//주품디자인 연장근로 강제 수동입력 160824
	if($com_code == 20695) $check_manual_ext = "checked";
	//$check_manual_night = "checked";
											...
											<div id="spanMain" style="width:100%;height:<?=$spanMain_height?>px;overflow-x:hidden;overflow-y:auto;" onScroll="spanLeft.scrollTop = this.scrollTop; spanTop.scrollLeft = this.scrollLeft;">
												<table width="<?=$pay_list_width?>" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:fixed;">
													...
													for ($i=0; $row=sql_fetch_array($result); $i++) {
														...
														//급여관리 DB (급여반영) 년월 / 급여복사 추가 160822
														if($data == "copy") {
															$sql4 = " select * from pibohum_base_pay where com_code='$row[com_code]' and sabun='$row[sabun]' and year = '$copy_year' and month = '$copy_month' and w_date != '0000-00-00' order by w_date desc, w_time desc limit 0, 1 ";
														} else {
...
if($row4['money_month'] == 0 || $data == "load") {
	...
	for($j=0; $row_attendance=sql_fetch_array($result_attendance); $j++) {
		...
		if($com_code == 20623) {
			//휴일근로 기간
			//휴일근로 기간1 전월
			if($row_com_opt['pay_calculate_day1_hday'] == "전월") {
				if($search_month == 1) {
					$pay_calculate_year1_hday = ($search_year-1);
					$pay_calculate_month1_hday = 12;
				} else {
					$pay_calculate_year1_hday = $search_year;
					$pay_calculate_month1_hday = ($search_month-1);
				}
			} else {
				$pay_calculate_year1_hday = $search_year;
				$pay_calculate_month1_hday = $search_month;
			}
			//휴일근로 기간2 익월
			if($row_com_opt['pay_calculate_day2_hday'] == "익월") {
				if($search_month == 12) {
					$pay_calculate_year2_hday = ($search_year+1);
					$pay_calculate_month2_hday = 1;
				} else {
					$pay_calculate_year2_hday = $search_year;
					$pay_calculate_month2_hday = ($search_month+1);
				}
			} else {
				$pay_calculate_year2_hday = $search_year;
				$pay_calculate_month2_hday = $search_month;
			}
			//근태일자와 휴일근로 기간 매치
			if( ($att_date[0] == $pay_calculate_year1_hday && $att_date[1] == $pay_calculate_month1_hday && $att_date[2]>=$row_com_opt['pay_calculate_day_period1_hday'] && $att_date[2]<=31) || ($att_date[0] == $pay_calculate_year2_hday && $att_date[1] == $pay_calculate_month2_hday && $att_date[2]>=1 && $att_date[2]<=$row_com_opt['pay_calculate_day_period2_hday']) ) {
				...
				//반올림
				$w_late = round($w_late);
				$w_leave = round($w_leave);
				$w_out = round($w_out);
				$w_absence = round($w_absence);
				$workhour_hday = round($workhour_hday);
			}
		//기본 사업장
		} else {
