...
$sub_title = "급여대장 : 사무직";
$g4[title] = $sub_title." : 급여관리 : ".$member['mb_nick'];
								...
								for ($i=0; $row=sql_fetch_array($result); $i++) {
									...
									if($search_month || $year != $old_year || $month != $old_month || $i == 0) {
										...
										if($member['mb_profile'] == "guest") {
											...
											$url_pay_ledger = "pay_white.php?w=u&code=".$code."&search_year=".$row[year]."&search_month=".$row[month]."&w_date=".$reg_day."&w_time=".$reg_time."&stx_dept=".$stx_dept;
											$url_pay_ledger_excel = "pay_ledger_excel.php?code=".$code."&search_year=".$row[year]."&search_month=".$row[month]."&w_date=".$reg_day."&w_time=".$reg_time;
										}
										if($reg_cnt > 1) {
											$url_pay_ledger_list = $_SERVER['PHP_SELF']."?search_year=".$row[year]."&search_month=".$row[month]."&stx_dept=".$stx_dept;
										} else {
...
if($member['mb_profile'] == "guest") {
	...
	$url_del = "javascript:checked_ok('white');";
