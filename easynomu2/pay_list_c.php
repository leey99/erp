											...
											<div id="spanMain" style="width:100%;height:<?=$spanMain_height?>px;overflow-x:hidden;overflow-y:auto;" onScroll="spanLeft.scrollTop = this.scrollTop; spanTop.scrollLeft = this.scrollLeft;">
												<table width="<?=$pay_list_width?>" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:fixed;">
													...
													for ($i=0; $row=sql_fetch_array($result); $i++) {
														...
														//급여관리 DB (급여반영) 년월
														$sql4 = " select * from pibohum_base_pay where com_code='$row[com_code]' and sabun='$row[sabun]' and year = '$search_year' and month = '$search_month' and w_date != '0000-00-00' order by w_date desc, w_time desc ";
														//echo $sql4;
														$result4 = sql_query($sql4);
														$row4=mysql_fetch_array($result4);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td>
			<textarea name="memo1" class="textfm" style='width:100%;height:38px;word-break:break-all;' onclick="if(this.value=='비고1'){this.value=''}"><?=$memo1?></textarea>
		</td>
		<td>
			<textarea name="memo2" class="textfm" style='width:100%;height:38px;word-break:break-all;' onclick="if(this.value=='비고2'){this.value=''}"><?=$memo2?></textarea>
		</td>
	</tr>
</table>
								<div style="height:10px"></div>
