...
//급여총액 0 제외 > 모든 활동보조인 표시 150716
//$sql_search .= " and money_total != 0 ";
//활동보조인 급여정보 추출 / 화성시장애인 20399  / 화성서남부 20627 활동보조인 코드 다름 160801
if($com_code == 20399) $sql_search .= " and position='13' ";
else if($com_code == 20627) $sql_search .= " and position='5' ";

//실지급액 존재 인원만 카운트 160801 -> 모든 근로자 포함 160926
...
$sub_title = "급여대장(활동보조인)";
$g4[title] = $sub_title." : 급여관리 : ".$easynomu_name;

//$sql_field = " com_code, sabun, year, month, dept, count(month) as cnt, sum(money_total) as sum_total, sum(money_gongje) as sum_gongje, sum(money_result) as sum_result, w_date, w_time ";
$sql_field = " com_code, sabun, year, month, dept, count(month) as cnt, sum(money_for_tax) as sum_total, sum(money_gongje) as sum_gongje, sum(money_result) as sum_result, w_date, w_time ";
	...
	if(cnt==0) {
	 ...
	 return;
		...
		if(confirm("정말 삭제하시겠습니까?")) {
			...
			//alert(chk_data);
			frm.action="pay_ledger_delete_beistand.php";
			frm.submit();
		} else {
			return;
		}
...
function printPayList(search_year, search_month, w_date, w_time) {
	...
	frm.target = "_blank";
	frm.action = "pay_ledger_beistand.php?code=<?=$com_code?>";
	frm.submit();
}
function month_plus() {
...
?>
							...
							<table width="100%" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:fixed;">
								<tr>
									<td nowrap class="tdhead_center" width="30"><input type="checkbox" name="checkall" value="1" class="textfm" onclick="checkAll()"></td>
<?
if($is_admin == "super") {
?>
...
?>
									...
									<td nowrap class="tdhead_center" width="100">성과금대장</td>
									<td nowrap class="tdhead_center" width="100">급여대장(엑셀)</td>
								...
								for ($i=0; $row=sql_fetch_array($result); $i++) {
									...
									if($search_month || $year != $old_year || $month != $old_month || $i == 0) {
										...
										if($member['mb_profile'] == "guest") {
											...
											//화성시장애인(직접입력) 화성서남부(엑셀업로드) 160803
											if($com_code != 20627) $url_pay_ledger = "pay_time_list.php?w=u&code=".$code."&search_year=".$row['year']."&search_month=".$row['month'];
											else $url_pay_ledger = "pay_excel_beistand_west.php?search_year=".$row['year']."&search_month=".$row['month'];
											$url_pay_ledger_excel = "pay_ledger_excel_beistand.php?code=".$code."&search_year=".$row['year']."&search_month=".$row['month'];
										}
										$url_pay_ledger_list = "pay_ledger_list_beistand.php?search_year=".$row['year']."&search_month=".$row['month'];
								?>
<?
if($search_month) {
	$w_gubun = "";
...
?>
									...
									<td nowrap class="ltrow1_center_h22"><strong><?=$row['year']?>년 <?=$row['month']?>월 급여대장</strong></td>
									...
									<td nowrap class="ltrow1_center_h22">
										<!--<a href="<?=$url_form?>"><img src="images/pay_ledger_hwp.png" border="0" alt="" style="margin:2px 0 3px 0;" /></a>-->
<?
if($com_code == 20399 && $row['year'] == 2016 && $row['month'] == "04") {
?>
										<a href="./excel/화성시장애인부모회_성과금_2015년도.xlsx"><img src="images/performance_fee_excel.png" border="0" alt="" style="margin:2px 0 3px 0;" /></a>
<?
}
?>
									</td>
									<td nowrap class="ltrow1_center_h22">
										<a href="<?=$url_pay_ledger_excel?>"><img src="images/pay_ledger_excel.png" border="0" alt="" style="margin:2px 0 3px 0;" /></a>
									</td>
									<td nowrap class="ltrow1_center_h22">
										<table border=0 cellpadding=0 cellspacing=0 style=display:inline;><tr><td width=2></td><td><img src=images/btn_lt.gif></td><td background=images/btn_bg.gif class=ftbutton1 nowrap><a href="<?=$url_pay_ledger?>">수정</a></td><td><img src=images/btn_rt.gif></td><td width=2></td></tr></table> 
									</td>
								</tr>
								</tr>
								<?
