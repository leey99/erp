...
function OnStart(){
	...
	if(pay_page > 1) pay_table = 'pay_ledger_g_'+pay_page+'page.hwp';
	else pay_table = 'pay_ledger_g.hwp';
	...
	if(!pHwpCtrl.Open(BasePath + "/files/docs/"+pay_table)) {
		...
		for(i=0;i<=tr_count;i++) {
			...
			if (pHwpCtrl.MoveToField("[연차수당"+i+"]", true, true, false)){set.SetItem("Text",frm.w_year[i].value);act.Execute(set);}
		...
		for(k=0;k<pay_page;k++) {
			...
			if (pHwpCtrl.MoveToField("[연차수당s"+m+"]", true, true, false)){set.SetItem("Text",frm.w_year_sum[k].value);act.Execute(set);}
