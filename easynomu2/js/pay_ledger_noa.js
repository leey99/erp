...
function OnStart(){
	...
	if(pay_page > 1) pay_table = 'pay_ledger_noa_'+pay_page+'page.hwp';
	else pay_table = 'pay_ledger_noa.hwp';
	...
	if(!pHwpCtrl.Open(BasePath + "/files/docs/"+pay_table)) {
		...
		for(k=1;k<=pay_page;k++) {
			...
			if (pHwpCtrl.MoveToField("기공2"+u, true, true, false)){set.SetItem("Text",frm.minus_text.value);act.Execute(set);}
		}
		tr_count = pay_page * 9;
		for(i=0;i<=tr_count;i++) {
			...
			if (pHwpCtrl.MoveToField("사번"+i, true, true, false)){set.SetItem("Text",frm.employee_no[i].value);act.Execute(set);}
