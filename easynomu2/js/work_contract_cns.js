	...
	switch(pName){
		case 'labor1': // 근로계약서
			if(!pHwpCtrl.Open(BasePath + "/files/docs/"+labor1)){
				...
				if (pHwpCtrl.MoveToField("[사업자등록번호1]", true, true, false)) {	set.SetItem("Text",frm.comp_num.value );	act.Execute(set);	}
				if (pHwpCtrl.MoveToField("[법인등록번호]", true, true, false)) {	set.SetItem("Text",frm.bupin_no.value );	act.Execute(set);	}
