...
function OnStart(){
	...
	if(pay_page > 1) pay_table = 'pay_ledger_c_'+pay_page+'page.hwp';
	else pay_table = 'pay_ledger_c.hwp';
	...
	if(!pHwpCtrl.Open(BasePath + "/files/docs/"+pay_table)) {
		...
		if (pHwpCtrl.MoveToField("비고1", true, true, false)){set.SetItem("Text",frm.memo1.value);act.Execute(set);}
		if (pHwpCtrl.MoveToField("비고2", true, true, false)){set.SetItem("Text",frm.memo2.value);act.Execute(set);}
	}
	pHwpCtrl.MovePos(2);
}
