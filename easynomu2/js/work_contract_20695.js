...
function pageLoad(pName){
	...
	if(frm.comp_type.value=='A') {
		if(frm.work_form.value == '2') {
			...
			if(frm.pay_gbn.value == '1') {
				labor1='labor_1a_timework.hwp';
			} else if(frm.pay_gbn.value == '3') {
				labor1='labor_1a_20695.hwp';
			} else {
				labor1='labor_1a.hwp';
			}
		}
	...
	switch(pName){
		case 'labor1': // 근로계약서
			if(!pHwpCtrl.Open(BasePath + "/files/docs/"+labor1)){
				...
				if (pHwpCtrl.MoveToField("연장근무", true, true, false)) {	set.SetItem("Text",frm.ext_time.value );	act.Execute(set);	}
				...
				if (pHwpCtrl.MoveToField("[토요일근무]", true, true, false)) {	set.SetItem("Text",frm.saturday_time.value );	act.Execute(set);	}
				...
				if (pHwpCtrl.MoveToField("[급여체계]", true, true, false)) {set.SetItem("Text",frm.pay_gbn_txt.value);act.Execute(set);}
				...
				if (pHwpCtrl.MoveToField("상여횟수", true, true, false)) {set.SetItem("Text",frm.bonus_time.value);act.Execute(set);}
				...
				if (pHwpCtrl.MoveToField("[차량유지]", true, true, false)) {	set.SetItem("Text",frm.pay_car.value );	act.Execute(set);	}
				if (pHwpCtrl.MoveToField("[식대]", true, true, false)) {	set.SetItem("Text",frm.pay_board.value );	act.Execute(set);	}
				if (pHwpCtrl.MoveToField("[육아보조]", true, true, false)) {	set.SetItem("Text",frm.pay_child_care.value );	act.Execute(set);	}
				if (pHwpCtrl.MoveToField("[e5]", true, true, false)) {	set.SetItem("Text",frm.pay_e5.value );	act.Execute(set);	}
				if (pHwpCtrl.MoveToField("[e6]", true, true, false)) {	set.SetItem("Text",frm.pay_e6.value );	act.Execute(set);	}
