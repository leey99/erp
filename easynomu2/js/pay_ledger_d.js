...
function replacecomma(restring){  //  , 제거 //
    if(restring.search(',')){
        exp=restring.split(',');
        for(t=0 ; ; t++){
            if(!exp[t]) break; 
...
function OnStart(){
	...
	if(pay_page > 1) pay_table = 'pay_ledger_d_'+pay_page+'page.hwp';
	else pay_table = 'pay_ledger_d.hwp';
	...
	if(!pHwpCtrl.Open(BasePath + "/files/docs/"+pay_table)) {
		...
		for(k=1;k<=pay_page;k++) {
			...
			if (pHwpCtrl.MoveToField("기타6"+u, true, true, false)){set.SetItem("Text",frm.b7_text.value);act.Execute(set);}
		...
		for(i=0;i<=tr_count;i++) {
			...
			frm.b6[i].value = replacecomma(frm.b6[i].value);
			frm.b7[i].value = replacecomma(frm.b7[i].value);
			frm.b6[i].value = Number(frm.b6[i].value) + Number(frm.b7[i].value);
			frm.b6[i].value = commastring(frm.b6[i].value);
		...
		for(k=0;k<pay_page;k++) {
			...
			frm.b6_sum[k].value = replacecomma(frm.b6_sum[k].value);
			frm.b7_sum[k].value = replacecomma(frm.b7_sum[k].value);
			frm.b6_sum[k].value = Number(frm.b6_sum[k].value) + Number(frm.b7_sum[k].value);
			frm.b6_sum[k].value = commastring(frm.b6_sum[k].value);
			...
			if (pHwpCtrl.MoveToField("기타공제2s"+m, true, true, false)){set.SetItem("Text",frm.minus2_sum[k].value);act.Execute(set);}
		...
		if (pHwpCtrl.MoveToField("통상임금계t", true, true, false)){set.SetItem("Text",frm.g_sum_sum_t.value);act.Execute(set);}
			...
			frm.b6_sum_t.value = replacecomma(frm.b6_sum_t.value);
			frm.b7_sum_t.value = replacecomma(frm.b7_sum_t.value);
			frm.b6_sum_t.value = Number(frm.b6_sum_t.value) + Number(frm.b7_sum_t.value);
			frm.b6_sum_t.value = commastring(frm.b6_sum_t.value);
