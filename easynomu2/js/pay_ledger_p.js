...
function OnStart(){
	...
	//연월 변수 지정, 2016년 2월 급여대장 건강정산, 요양정산 추가 160308
	var search_year = frm.search_year.value;
	var search_month = frm.search_month.value;

	if(pay_page > 1) pay_table = 'pay_ledger_p_'+pay_page+'page.hwp';
	else pay_table = 'pay_ledger_p.hwp';

	if(search_year == 2016 && (search_month == "02" || search_month == "03")) pay_table = 'pay_ledger_p_2016_02.hwp';
	...
	if(!pHwpCtrl.Open(BasePath + "/files/docs/"+pay_table)) {
		...
		for(k=1;k<=pay_page;k++) {
			...
			if (pHwpCtrl.MoveToField("기공3"+u, true, true, false)){set.SetItem("Text",frm.minus3_text.value);act.Execute(set);}
		}
		tr_count = pay_page * 9;
		for(i=0;i<=tr_count;i++) {
			...
			if (pHwpCtrl.MoveToField("생리수당"+i, true, true, false)){set.SetItem("Text",frm.money_period[i].value);act.Execute(set);}
			...
			if (pHwpCtrl.MoveToField("기타공제2_"+i, true, true, false)){set.SetItem("Text",frm.c_health[i].value);act.Execute(set);}
			if (pHwpCtrl.MoveToField("기타공제3_"+i, true, true, false)){set.SetItem("Text",frm.c_yoyang[i].value);act.Execute(set);}
		...
		for(k=0;k<pay_page;k++) {
			...
			if (pHwpCtrl.MoveToField("[추가야간s"+m+"]", true, true, false)){set.SetItem("Text",frm.w_night_add_sum[k].value);act.Execute(set);}
			if (pHwpCtrl.MoveToField("[추가휴일s"+m+"]", true, true, false)){set.SetItem("Text",frm.w_hday_add_sum[k].value);act.Execute(set);}
			if (pHwpCtrl.MoveToField("[근태공제s"+m+"]", true, true, false)){set.SetItem("Text",frm.w_etc_sum[k].value);act.Execute(set);}
			...
			if (pHwpCtrl.MoveToField("생리수당s"+m, true, true, false)){set.SetItem("Text",frm.money_period_sum[k].value);act.Execute(set);}
			...
			if (pHwpCtrl.MoveToField("기타3s"+m, true, true, false)){set.SetItem("Text",frm.b3_sum[k].value);act.Execute(set);}
			if (pHwpCtrl.MoveToField("기타4s"+m, true, true, false)){set.SetItem("Text",frm.b4_sum[k].value);act.Execute(set);}
			if (pHwpCtrl.MoveToField("기타5s"+m, true, true, false)){set.SetItem("Text",frm.b5_sum[k].value);act.Execute(set);}
			...
			if (pHwpCtrl.MoveToField("가불s"+m, true, true, false)){set.SetItem("Text",frm.etc_sum[k].value);act.Execute(set);}
			if (pHwpCtrl.MoveToField("근태공제s"+m, true, true, false)){set.SetItem("Text",frm.etc2_sum[k].value);act.Execute(set);}
			...
			if (pHwpCtrl.MoveToField("기타공제2s"+m, true, true, false)){set.SetItem("Text",frm.c_health_sum[k].value);act.Execute(set);}
			if (pHwpCtrl.MoveToField("기타공제3s"+m, true, true, false)){set.SetItem("Text",frm.c_yoyang_sum[k].value);act.Execute(set);}
		...
		if (pHwpCtrl.MoveToField("생리수당t", true, true, false)){set.SetItem("Text",frm.period_sum_t.value);act.Execute(set);}
		...
		if (pHwpCtrl.MoveToField("기타공제2t", true, true, false)){set.SetItem("Text",frm.c_health_sum_t.value);act.Execute(set);}
		if (pHwpCtrl.MoveToField("기타공제3t", true, true, false)){set.SetItem("Text",frm.c_yoyang_sum_t.value);act.Execute(set);}
