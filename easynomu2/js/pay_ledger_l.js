...
function OnStart(){
	...
	if(pay_page > 1) pay_table = 'pay_ledger_l_'+pay_page+'page.hwp';
	else pay_table = 'pay_ledger_l.hwp'
	...
	if(!pHwpCtrl.Open(BasePath + "/files/docs/"+pay_table)) {
		...
		for(k=0;k<pay_page;k++) {
			...
			if (pHwpCtrl.MoveToField("기타9s"+m, true, true, false)){set.SetItem("Text",frm.b9_sum[k].value);act.Execute(set);}
