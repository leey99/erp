...
$sql = " select com_code, sabun, year, month, count(month) as cnt, sum(money_total) as sum_total, sum(money_gongje) as sum_gongje, sum(money_result) as sum_result, w_date, w_time, dept_code
          $sql_common
          $sql_search $sql_group
          $sql_order 
...
function printPayList(search_year, search_month, w_date, w_time, dept_code) {
...
?>
								...
								for ($i=0; $row=sql_fetch_array($result); $i++) {
									...
									if($search_month || $year != $old_year || $month != $old_month || $i == 0) {
										...
										//echo " // ".$no;
										//부서코드
										//if($row['dept_code'] >= 2 && $row['dept_code'] <= 6) $dept_code = 2;
										//else $dept_code = $row['dept_code'];
										...
										if($member['mb_profile'] == "guest") {
											...
											$url_form = "javascript:printPayList('$row[year]','$row[month]','$reg_day','$reg_time','$dept_code');";
											$url_pay_ledger = "pay_blue.php?w=u&code=".$code."&search_year=".$row[year]."&search_month=".$row[month]."&w_date=".$reg_day."&w_time=".$reg_time;
											$url_pay_ledger_excel = "pay_ledger_excel.php?code=".$code."&search_year=".$row[year]."&search_month=".$row[month]."&w_date=".$reg_day."&w_time=".$reg_time."&dept_code=".$dept_code;
										}
										if($reg_cnt > 1) {
											$url_pay_ledger_list = $_SERVER['PHP_SELF']."?search_year=".$row[year]."&search_month=".$row[month]."&stx_dept=".$dept_code;
										} else {
											$url_pay_ledger_list = "javascript:alert('추가 저장 급여대장은 없습니다.');";
										}
								?>
<?
if($search_month) {
...
if($member['mb_profile'] == "guest") {
	...
	$url_del = "javascript:checked_ok('blue');";
