...
function goInput(mode){
	...
	*/
	...
	f.mode.value = mode;
	f.action = "pay_update_type_h_month.php";
	f.submit();
}
//급여반영 입력폼
...
function check_manual(m_name) {
	...
	for(i=1;i<=total_cnt;i++) {
		...
		} else if(m_name.name == "manual_retirement_pension") {
			if(m_name.checked) {
				frm.retirement_pension[i].className = "textfm";
				frm.retirement_pension[i].readOnly = false;
			} else {
				frm.retirement_pension[i].className = "textfm5";
				frm.retirement_pension[i].readOnly = true;
			}
/*
...
function cal_pay2(idx){
	...
	var week_hday,year_hday,money_base,money_ext,money_night,money_hday,money_ext_add,money_night_add,money_hday_add,money_year;
...
function focusOutClass(idx) {
	...
	if(frm.idx[idx].checked == false) {
		...
		//퇴직연금 회색 처리 여부 161216
		if(frm.manual_retirement_pension.checked) frm.retirement_pension[idx].className = "textfm";
		else frm.retirement_pension[idx].className = "textfm5";
...
<?
include "./inc/pay_list_include_h_month.php";
?>
								<!--리스트 -->
														...
														<td>  
															<table width="<?=$pay_list_width?>" border="0" cellpadding="0" cellspacing="1" class="bgtable" style="table-layout:;">
																<tr>
																	...
																	<td class="tdhead_center" rowspan="4" width="100"><input type="checkbox" name="manual_retirement_pension" <?=$check_manual_retirement_pension?> onclick="check_manual(this);" value="1" title="수동입력" style="vertical-align:middle;"> 퇴직연금</td>
																	<td class="tdhead_center" rowspan="4" width="18"></td>
																</tr>
																<tr>
																	...
																	<td class="tdhead_center" rowspan="3" width="100">공제계</td>
																</tr>
																<tr>
																	...
																	<td class="tdhead_center" rowspan="2" width="60">기타공제</td>

																	<td class="tdhead_center" rowspan="2" width="98">국민연금</td>
																	<td class="tdhead_center" rowspan="2" width="98">건강보험</td>
																	<td class="tdhead_center" rowspan="2" width="98">장기요양</td>
																	<td class="tdhead_center" rowspan="2" width="97">고용보험</td>
																	<td class="tdhead_center" rowspan="2" width="97">산재보험</td>
																</tr>
																<tr>
...
?>
											<div id="spanLeft" style="width:200px;height:<?=$spanMain_height?>px;overflow:hidden">
												<div style="display:none;">
													...
													<!--채용형태-->
													<input type="hidden" name="workform">
													<!--두리누리-->
...
?>
														...
														<!--채용형태-->
														<input type="hidden" name="workform" value="<?=$work_form?>" />
														<!--두리누리-->
...
?>
														...
														<td nowrap class="ltrow1_center_h24" width="97"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber_negative();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="c_money_health" id="id_c_money_health_<?=$k?>" value="<?=number_format($row4['c_health'])?>" onkeyup="checkThousand_negative(this.value, this,'Y');cal_pay2('<?=$k?>');" onkeydown="if(event.keyCode == 13){ getId('id_c_money_health_<?=$k_next?>').focus(); }" /></td><!--건강보험-->
														<td nowrap class="ltrow1_center_h24" width="97"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="onlyNumber_negative();" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm" name="c_money_yoyang" id="id_c_money_yoyang_<?=$k?>" value="<?=number_format($row4['c_yoyang'])?>" onkeyup="checkThousand_negative(this.value, this,'Y');cal_pay2('<?=$k?>');" onkeydown="if(event.keyCode == 13){ getId('id_c_money_yoyang_<?=$k_next?>').focus(); }" /></td><!--장기요양보험-->
														...
														<td nowrap class="ltrow1_center_h24" width="99"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm5" readonly name="c_money_gongje" value="<?=number_format($row4['c_money_gongje'])?>"></td><!--공제계-->
														<td nowrap class="ltrow1_center_h24" width="100"><input type="text" style="width:100%;ime-mode:disabled;text-align:right" onKeyPress="" onfocusin="javascript:focusInClass('<?=$k?>');" onfocusout="javascript:focusOutClass('<?=$k?>');" class="textfm5" readonly name="retirement_pension" value="<?=number_format($row4['retirement_pension'])?>"></td><!--퇴직연금-->
														<td nowrap class="ltrow1_center_h24" width="18"></td>
													</tr>
...
function cal_pay(idx) {
	var f = document.dataForm;
	var pay_gbn,money_day, c_money_yun,c_money_health,c_money_yoyang,c_money_goyong,c_money_sanjae,c_money_gongje;
	...
	var week_hday,year_hday,money_base,money_time,money_ext,money_night,money_hday,money_ext_add,money_night_add,money_hday_add,money_year;
	var money_g1,money_g2,money_g3,money_g4,money_g5,money_e1,money_e2,money_e3,money_e4,money_e5,money_e6,money_e7,money_e8;
...
?>
	...
	if(!f.manual_4insure.checked) {
		...
		//money_health = get_round( parseInt(money_for_tax) * 0.02995 ); //2014년
		//money_health = get_round( parseInt(money_for_tax) * 0.03035 ); //2015년
		money_health = get_round( parseInt(money_for_tax) * 0.0306 ); //2016년
		//money_yoyang 장기요양보험 
	...
	//4대보험 수동입력 유무(사업장)
	if(!f.c_manual_4insure.checked) {
		c_money_yun = get_round( parseInt(money_for_tax) * 0.045 / durunuri_50 );
		//건강보험 계산(사업장)
		c_money_health = get_round( parseInt(money_for_tax) * 0.03035 );
		//money_yoyang 장기요양보험 
		c_money_yoyang = get_round( money_health* 0.0655  );
		//money_goyong 고용보험
		c_money_goyong = get_round( parseInt(money_for_tax) * 0.0065 / durunuri_50 );
		//산재보험
		c_money_sanjae = get_round( parseInt(money_for_tax) * 0.00784 / durunuri_50 );
		//4대보험 공제 제외 (사업장)
