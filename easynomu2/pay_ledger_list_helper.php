...
$sql = " select com_code, sabun, year, month, dept, count(month) as cnt, sum(money_for_tax) as sum_total, sum(money_gongje) as sum_gongje, sum(money_result) as sum_result, w_date, w_time
          $sql_common
          $sql_search $sql_group
          $sql_order 
	...
	if(cnt==0) {
	 ...
	 return;
		...
		if(confirm("정말 삭제하시겠습니까?")) {
			...
			//alert(chk_data);
			frm.action="pay_ledger_delete_helper.php";
			frm.submit();
		} else {
			return;
		}
...
function printPayList(search_year, search_month, w_date, w_time) {
	...
	frm.target = "_blank";
	frm.action = "pay_ledger_helper.php?code=<?=$com_code?>";
	frm.submit();
}
function month_plus() {
...
?>
									...
									<td nowrap class="tdhead_center" width="100">급여대장(간편)</td>
								...
								for ($i=0; $row=sql_fetch_array($result); $i++) {
									...
									if($search_month || $year != $old_year || $month != $old_month || $i == 0) {
										...
										if($member['mb_profile'] == "guest") {
											...
											$url_pay_ledger = "pay_excel_helper.php?w=u&code=".$code."&search_year=".$row['year']."&search_month=".$row['month'];
											$url_pay_ledger_excel = "pay_ledger_excel_helper.php?code=".$code."&search_year=".$row['year']."&search_month=".$row['month'];
										}
										$url_pay_ledger_list = "pay_ledger_list_helper.php?search_year=".$row['year']."&search_month=".$row['month'];
								?>
<?
if($search_month) {
	$w_gubun = "";
