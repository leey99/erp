...
//부서별 구분
//$sql_search .= " and ( b.dept >= 2 and b.dept <= 6 ) ";
...
if($row_com_opt['pay_day_time']) {
	if($search_month == 12) {
		...
		$pay_date = number_format($search_month+1)."월 ".$row_com_opt['pay_day_time']."일";
	}
	//급여지급일 당월 체크 유무
	if($row_com_opt['pay_day_now_month_time'] == 1) {
		$pay_date = number_format($search_month)."월 ".$row_com_opt['pay_day_time']."일";
	}
} else {
	$pay_date = "급여지급일 미지정";
}
//임금산출기간
//임금산출기간1 전월
if($row_com_opt['pay_calculate_day1_time'] == "전월") {
	if($search_month == 1) {
...
//임금산출기간2 익월
if($row_com_opt['pay_calculate_day2_time'] == "익월") {
	if($search_month == 12) {
...
$pay_calculate_date = $pay_calculate_year1."년 ".number_format($pay_calculate_month1)."월 ".$row_com_opt['pay_calculate_day_period1_time']."일 ~ ".$pay_calculate_year2."년 ".number_format($pay_calculate_month2)."월 ".$row_com_opt['pay_calculate_day_period2_time']."일";
?>
<head>
