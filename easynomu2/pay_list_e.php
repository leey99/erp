...
<?
//대성하이테크 부양가족 소득세 적용 제외
if($member['mb_id'] = "123-81-61478") {
?>
...
function cal_pay2(idx){
	...
	var money_total,money_for_tax,money_yun,money_health,money_yoyang,money_goyong,tax_so,tax_jumin,minus,money_gongje,money_result, workhour_year;
...
?>
														<td nowrap class="ltrow1_center_h24"><b onclick="clientxy();emp_text('<?=$row[name]?>','<?=$in_day?>','<?=$row_position[name]?>','<?=$row2[family_cnt]?>','<?=$row2[work_gbn]?>','<?=$pay_gbn.$input_type?>','<?=number_format($row4['money_month_base'])?>','<?=number_format($row4['money_hour_ms'])?>','<?=number_format($row4['money_min_base'])?>');" style="cursor:pointer"><?=$name?></b></td>
