...
// 천단위 콤마 제거 DB 저장
$join_salary = ereg_replace(',', '', $join_salary);
$quit_sum_now = ereg_replace(',', '', $quit_sum_now);
$quit_sum_pre = ereg_replace(',', '', $quit_sum_pre);
$quit_3month = ereg_replace(',', '', $quit_3month);
for($i=0; $i<=3; $i++) {
	$join_salary_[$i] = ereg_replace(',', '', $join_salary_[$i]);
	$quit_sum_now_[$i] = ereg_replace(',', '', $quit_sum_now_[$i]);
	$quit_sum_pre_[$i] = ereg_replace(',', '', $quit_sum_pre_[$i]);
	$quit_3month_[$i] = ereg_replace(',', '', $quit_3month_[$i]);
}
...
if ($w == 'u'){
	...
	$sql = " insert $g4[insure_table] set $sql_common ";
	sql_query($sql);
	$id = mysql_insert_id();
	...
	$subject = "[이지노무] 4대보험 취득/상실/변경 신고 접수 되었습니다.";
	$content = "<span style='font-size:9pt;'>[이지노무] ".$comp_name." 4대보험 취득/상실/변경 신고 접수 되었습니다.<p>";
	$content .= "<a href=\"http://www.easynomu.com/easynomu/4insure_list.php\">☞ 4대보험 취득/상실/변경 신고 접수 리스트</a></p><p>아이디 : master / 패스워드 : 8430<p></p>".date("Y-m-d H:i:s")."<p>이 메일 주소로는 회신되지 않습니다.</p></span>";
	...
	$kind = 1;
	if($join_code) {
		$sql_history = " insert join_quit_history set com_code='$com_code', sabun='$join_code', name='$join_name', kind='$kind', jq_date='$join_date', reg_time='$now_time' ";
		sql_query($sql_history);
	}
	for($i=0;$i<=3;$i++) {
		if($join_code_[$i]) {
			$sql_history = " insert join_quit_history set com_code='$com_code', sabun='$join_code_[$i]', name='$join_name_[$i]', kind='$kind', jq_date='$join_date_[$i]', reg_time='$now_time' ";
			sql_query($sql_history);
		}
	}
	//퇴사일 자동 등록 151104
	$kind = 2;
	if($quit_code) {
		$sql_history = " insert join_quit_history set com_code='$com_code', sabun='$quit_code', name='$quit_name', kind='$kind', jq_date='$quit_date', reg_time='$now_time' ";
		sql_query($sql_history);
	}
	for($i=0;$i<=3;$i++) {
		if($quit_code_[$i]) {
			$sql_history = " insert join_quit_history set com_code='$com_code', sabun='$quit_code_[$i]', name='$quit_name_[$i]', kind='$kind', jq_date='$quit_date_[$i]', reg_time='$now_time' ";
			sql_query($sql_history);
		}
	}
}
