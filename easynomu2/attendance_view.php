...
<?
$att_day_date = str_replace('-','.',$att_day_txt);
//사원정보
$sql1 = " select * from $g4[pibohum_base] where com_code='$com_code' and !(out_day!='' and out_day<'$att_day_date') ";
//echo $sql1;
$result1 = sql_query($sql1);
for($i=0; $row1=sql_fetch_array($result1); $i++) {
	$sabun_name[$row1[sabun]] = $row1[name];
?>
													<option value='<?=$row1[sabun]?>' <? if($row1[sabun] == $row_att[sabun]) echo "selected"; ?> ><?=$row1[name]?> (<?=$row1[in_day]?>)</option>
<?
}
?>
												</select>
...
if($member['mb_profile'] == "guest") {
	...
	$url_del = "javascript:del($id,$toYear,$toMonth);";
}
?>
...
for($i=0; $row=sql_fetch_array($result); $i++) {
	...
	if($member['mb_profile'] == "guest") {
		...
		$url_modify = "javascript:modify($id,$toYear,$toMonth,$monthday');";
		$url_del = "javascript:del($id,$toYear,$toMonth);";
	}
?>
								<tr class="list_row_now_wh" onMouseOver="this.className='list_row_over';" onMouseOut="this.className='list_row_now_wh';">
									...
									<td nowrap class="ltrow1_center_h22">
										<div id="btn_bsget_82">
										 <table border=0 cellpadding=0 cellspacing=0 style=display:inline;><tr><td width=2></td><td><img src=images/btn1_lt.gif></td><td background=images/btn1_bg.gif class=ftbutton3_white nowrap><a href="<?=$url_modify?>" target="">수정</a></td><td><img src=images/btn1_rt.gif></td><td width=2></td></tr></table>
										</div>
									</td>
									<td nowrap class="ltrow1_center_h22">
										<div id="btn_bslost_82">
										 <table border=0 cellpadding=0 cellspacing=0 style=display:inline;><tr><td width=2></td><td><img src=images/btn1_lt.gif></td><td background=images/btn1_bg.gif class=ftbutton3_white nowrap><a href="<?=$url_del?>" target="">삭제</a></td><td><img src=images/btn1_rt.gif></td><td width=2></td></tr></table>
										</div>
									</td>
								</tr>
<?
